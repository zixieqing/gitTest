...
function RandomDriver:RandomWeakEffect(skillId, weakPoints, weekPointIdx)
	...
	local r = G_BattleLogicMgr:GetRandomManager():GetRandomInt(fixedRange)

	-- 如果上一次是无效果 为打断随机一个叠加几率 无效果几率相对减少
	if self.weekEffectCounter > 0 then
		self.weekEffectFixedValue = self.weekEffectFixedValue +
		G_BattleLogicMgr:GetRandomManager():GetRandomIntByRange(self.weekEffectOverlayConf.overlayMin * fixedRange, self.weekEffectOverlayConf.overlayMax * fixedRange)
	end
	...
	elseif nil ~= weakPointIdMap[tostring(ConfigWeakPointId.HALF_EFFECT)] and
		...
		-- 减半 不会修正
		-- 减半需要取一次效果值
		local randomIdx = weakPointIdMap[tostring(ConfigWeakPointId.HALF_EFFECT)][G_BattleLogicMgr:GetRandomManager():GetRandomInt(#weakPointIdMap[tostring(ConfigWeakPointId.HALF_EFFECT)] - 1) + 1]
	...
	elseif nil ~= weakPointIdMap[tostring(ConfigWeakPointId.BREAK)] and
		...
		-- 二次随机
		local randomFloat = G_BattleLogicMgr:GetRandomManager():GetRandomFloat(1)
