--[[
 * author : liuzhipeng
 * descpt : 活动 组合活动 抽奖View
--]]
local AssemblyActivityLotteryView = class('AssemblyActivityLotteryView', function ()
    local node = CLayout:create(display.size)
    node.name = 'Game.views.activity.assemblyActivity.AssemblyActivityLotteryView'
...
local RES_DICT = {
    ...
    LOTTERY_LAYOUT_BG               = _res('ui/springActivity20/lottery/wonderland_draw_bg_down.png'),
	DRAW_BTN_N                      = _res("ui/home/capsuleNew/common/summon_newhand_btn_draw.png"),
    DRAW_BTN_D                      = _res("ui/home/capsuleNew/newPlayerCapsule/summon_newhand_btn_draw_locked.png"),
    LOTTERY_CONSUME_BG              = _res("ui/anniversary19/lottery/wonderland_draw_label_num.png"),
    LOTTERY_DESCR_LABEL_BG          = _res("ui/anniversary19/lottery/wonderland_draw_label_text.png"),
    DRAW_PROBABILITY_BTN            = _res('ui/home/capsule/draw_probability_btn.png'),
    COMMON_BG_GOODS                 = _res('ui/common/common_bg_goods'),
    RARE_REWARDS_BTN                = _res('ui/common/tower_btn_quit.png'),
    RARE_REWARDS_BTN_NOTICE         = _res('avatar/ui/restaurant_btn_festival_notice'),
    SPLIT_LINE                      = _res('ui/anniversary19/lottery/wonderland_draw_line_subtitle.png'),
    LIST_VIEW_SPLIT_LINE            = _res('ui/common/season_loots_line_1'),
    REWARD_CELL_COMMON              = _res('ui/home/activity/seasonlive/season_loots_label_goods'),
    REWARD_CELL_RARE                = _res('ui/anniversary19/lottery/season_loots_label_goods_rare.png'),
    REWARD_CELL_DISABLE             = _res('ui/home/activity/seasonlive/season_loots_label_goods_bk'),
}
function AssemblyActivityLotteryView:ctor( params )
    self.npc = params.npc or 200001
    self.cartoon = params.cartoon
    self.cartoon2 = params.cartoon2
...
init ui
--]]
function AssemblyActivityLotteryView:InitUI()
    local function CreateView()
		...
		display.commonLabelParams(tabNameLabel, {ttf = true, font = TTF_GAME_FONT, text = __('返场狂欢会'), fontSize = 30, color = '473227',offset = cc.p(0,-8)})
        ...
        display.commonLabelParams(probabilityBtn, fontWithColor(18, {text = __('概率') ,reqW = 140}))
        -- 稀有奖励
        ...
        display.commonLabelParams(rareRewardBtn, {text = __('稀有奖励一览'), fontSize = 22, color = '#ffffff'})
        ...
        display.commonLabelParams(drawOneBtn, {text = __('换1个'), fontSize = 24, color = '#ffffff', font = TTF_GAME_FONT, ttf = true, outline = '#5c5c5c', outlineSize = 2})
        ...
        display.commonLabelParams(drawTenBtn, {text = __('换10个'), fontSize = 24, color = '#ffffff', font = TTF_GAME_FONT, ttf = true, outline = '#5c5c5c', outlineSize = 2})
        ...
        local rabbitSpine = sp.SkeletonAnimation:create(
            string.format('ui/home/activity/assemblyActivity/lottery/%s.json', self.cartoon2),
            string.format('ui/home/activity/assemblyActivity/lottery/%s.atlas', self.cartoon2),
        1)
        ...
        -- 卡牌切图
        local cardLayoutBg = display.newImageView(_res(string.format('ui/home/activity/assemblyActivity/lottery/wonderland_draw_bg_reward_%d.png', self.npc)), cardLayoutSize.width / 2, cardLayoutSize.height / 2)
        cardLayout:addChild(cardLayoutBg, 1)
        -- 卡牌预览
        local cardPreviewBtn = require("common.CardPreviewEntranceNode").new({confId = self.npc})
        ...
        local rewardSpine = sp.SkeletonAnimation:create(
            string.format('ui/home/activity/assemblyActivity/lottery/%s.json', self.cartoon),
            string.format('ui/home/activity/assemblyActivity/lottery/%s.atlas',self.cartoon),
        1)
		...
		-- money barBg
		local moneyBarBg = display.newImageView(RES_DICT.MONEY_INFO_BAR, display.width, display.height, {ap = display.RIGHT_TOP, scale9 = true, size = cc.size(1, 54)})
...
隐藏UI
--]]
function AssemblyActivityLotteryView:HideUI()
...
进入动画
--]]
function AssemblyActivityLotteryView:EnterAction(  )
    -- 添加点击屏蔽
...
刷新道具数量
--]]
function AssemblyActivityLotteryView:UpdateGoodsNum()
	self:UpdateMoneyBar()
end
--[[
重载货币栏
--]]
function AssemblyActivityLotteryView:ReloadMoneyBar(moneyIdMap, isDisableGain)
    if moneyIdMap then
    ...
    for i = #moneyIdList, 1, -1 do
		...
		local isShowHpTips = moneyId == HP_ID and 1 or -1
...
刷新货币栏
--]]
function AssemblyActivityLotteryView:UpdateMoneyBar()
...
--]]
function AssemblyActivityLotteryView:RefreshLotteryLayout( params )
    local viewData = self:GetViewData()
    local goodsConf = CommonUtils.GetConfig('goods', 'goods', params.consume.goodsId) or {}
    viewData.lotteryDescrLabel:setString(string.fmt(__('你愿意用_name_交换礼物吗？'), {['_name_'] = tostring(goodsConf.name)}))
    display.reloadRichLabel(viewData.drawOneConsumeLabel, {c = {
        {text = string.fmt(__('消耗_num_'), {['_num_'] = params.consume.num}), fontSize = 20, color = '#ffffff'},
    ...
    display.reloadRichLabel(viewData.drawTenConsumeLabel, {c = {
        {text = string.fmt(__('消耗_num_'), {['_num_'] = checkint(params.consume.num) * 10}), fontSize = 20, color = '#ffffff'},
        {img = CommonUtils.GetGoodsIconPathById(params.consume.goodsId), scale = 0.18}
    }})
...
--]]
function AssemblyActivityLotteryView:RefreshRewardLayout( params )
    local viewData = self:GetViewData()
    viewData.turnLabel:setString(string.fmt(__('第_num_轮'), {['_num_'] = params.round}))
    viewData.leftNumLabel:setString(string.fmt(__('剩余：_num1_/_num2_'), {['_num1_'] = params.leftNum, ['_num2_'] = params.totalNum}))
...
--]]
function AssemblyActivityLotteryView:RefreshRewardListView( lotteryData )
    local viewData = self:GetViewData()
...
--]]
function AssemblyActivityLotteryView:CreateRewardsLayout( rewardsData, isRare )
    local viewData = self:GetViewData()
...
创建列表Cell
--]]
function AssemblyActivityLotteryView:CreateGridCell(data, isRare)
    data  = data or {}
    local bgImage = nil
    if isRare then
...
--]]
function AssemblyActivityLotteryView:ShowRewardSpine( times )
    local viewData = self:GetViewData()
    ...
    -- end
    viewData.rewardSpine:setAnimation(0, 'play', false)
end
--[[
隐藏奖励spine
--]]
function AssemblyActivityLotteryView:HideRewardSpine()
    local viewData = self:GetViewData()
...
获取viewData
--]]
function AssemblyActivityLotteryView:GetViewData()
    return self.viewData
end
return AssemblyActivityLotteryView