...
function ByteArray:getBytes(__offset, __length)
	...
	--cclog("getBytes,offset:%u, length:%u", __offset, __length)
...
function ByteArray:getPack(__offset, __length)
    ...
    if len >= 5000 then
        local tt = {}
        for i=1,len,5000 do
            local span = math.min(i+4999, len) - i + 1
            local __fmt = self:_getLC("b"..span)
            local tmp = string.pack(__fmt, unpack(__t, i, math.min(i+4999, len)))
            table.insert(tt, tmp)
        end
        __s = table.concat(tt,'')
    else
        __s = string.pack(__fmt, unpack(__t))
    end
    ------------ 处理报文过长 超过8000的内容unpack时会出错 ------------
...
function ByteArray:readBuf(__len)
	--cclog("readBuf,len:%u, pos:%u", __len, self._pos)
