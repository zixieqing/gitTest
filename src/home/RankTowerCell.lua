...
爬塔排行榜知名度排行Cell
--]]
local RankTowerCell = class('RankTowerCell', function ()
	local RankTowerCell = CGridViewCell:new()
	RankTowerCell.name = 'home.RankTowerCell'
	RankTowerCell:enableNodeEvents()
	return RankTowerCell
end)

function RankTowerCell:ctor( ... )
	local arg = { ... }
	local size = arg[1]
	...
	self.scoreNum:setPosition(940, size.height/2)
	self.scoreNum:setAnchorPoint(cc.p(1, 0.5))
	self.eventNode:addChild(self.scoreNum, 10)
	self.floorLabel = display.newLabel(942, size.height/2, fontWithColor(14, {text = __('层'),reqW  = 60 ,  ap = display.LEFT_CENTER}))
	self.eventNode:addChild(self.floorLabel, 10)
end
return RankTowerCell
