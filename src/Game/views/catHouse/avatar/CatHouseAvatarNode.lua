...
local CatHouseAvatarNode = class('CatHouseAvatarNode', function()
    local AIMG = 'ui/common/story_tranparent_bg.png'
    local node = display.newImageView(_res(AIMG))
    node.name  = 'CatHouseAvatarNode'
...
function CatHouseAvatarNode:ctor(args)
    -- parse args
...
function CatHouseAvatarNode:getViewData()
    return self.viewData_
end


function CatHouseAvatarNode:getAvatarId()
    return checkint(self.avatarId_)
end
function CatHouseAvatarNode:setAvatarId(avatarId)
...
function CatHouseAvatarNode:isAnimationNode()
...
function CatHouseAvatarNode:updateAvatar_()
    ...
    if self:getAvatarId() > 0 then
        -- update content size
        local locationConf = CONF.CAT_HOUSE.AVATAR_LOCATION:GetValue(self:getAvatarId())
        ...
        self.animationConf_ = CONF.CAT_HOUSE.AVATAR_ANIMATE:GetValue(self:getAvatarId())
        -------------------------------------------------
        -- update avatar spine
        if self:isAnimationNode() then
            local avatarSpinePath = AssetsUtils.GetCatHouseBigAvatarNode(self:getAvatarId())
        ...
        -- update avatar image
            ...
            local avatarImagePath = AssetsUtils.GetCatHouseBigAvatarPath(self:getAvatarId())
...
function CatHouseAvatarNode:updateParticle_()
    -- clean download task
...
function CatHouseAvatarNode:toNextAnimation_()
    local animationConf = self.animationConf_[self.animationName_] or {}
    self:switchAnimation_(animationConf.to)
end
function CatHouseAvatarNode:switchAnimation_(animationName)
    self.animationName_ = checkstr(animationName)
    self:updateAnimation_()
end


function CatHouseAvatarNode:updateAnimation_()
...
function CatHouseAvatarNode:checkAnimationEnded_()
...
function CatHouseAvatarNode:stopAnimationAudio_()
...
function CatHouseAvatarNode:playAnimationAudio_(audioId, audioTime)
    self:stopAnimationAudio_()
...
function CatHouseAvatarNode:createFullEffectAvatar_(fullEffectId)
...
function CatHouseAvatarNode:onEnter()
end


function CatHouseAvatarNode:onCleanup()
    -- stop animation audio
    self:stopAnimationAudio_()
...
function CatHouseAvatarNode:onResDownloadedHandler_(signal)
...
function CatHouseAvatarNode:onClickAvatarNodeHandler_(sender)
    if not self.isControllable_ then return end
    self:toNextAnimation_()
end


function CatHouseAvatarNode:onAnimationCompleteHandler_(event)
