	...
	if ConfigSeekTargetRule.T_OBJ_SELF == ttype then
		...
		if ConfigSeekTargetRule.T_OBJ_ALL == ttype then
			...
			for i = #BMediator:GetBData().sortBattleObjs.friend, 1, -1 do
				...
				if not obj:isLuck() and nil == obj:findInfectDriverExistBySkillId(infectSkillId) and true ~= ruleOutTags_[tostring(obj:getOTag())] then
					table.insert(result, obj)
				end
			end
			for i = #BMediator:GetBData().sortBattleObjs.enemy, 1, -1 do
				obj = BMediator:GetBData().sortBattleObjs.enemy[i]
				if not obj:isLuck() and nil == obj:findInfectDriverExistBySkillId(infectSkillId) and true ~= ruleOutTags_[tostring(obj:getOTag())] then
					table.insert(result, obj)
				end
			end
		...
		elseif ConfigSeekTargetRule.T_OBJ_FRIEND_PLAYER == ttype or ConfigSeekTargetRule.T_OBJ_ENEMY_PLAYER == ttype then
			-- 敌方或友方普通规则 这里的配表敌友性总是相对的
			if isEnemy then
				ttype = ConfigSeekTargetRule.T_OBJ_FRIEND_PLAYER + ConfigSeekTargetRule.T_OBJ_ENEMY_PLAYER - ttype
			end
			if ConfigSeekTargetRule.T_OBJ_ENEMY_PLAYER == ttype then
				for i = #BMediator:GetBData().sortPlayerObj.enemy, 1, -1 do
					obj = BMediator:GetBData().sortPlayerObj.enemy[i]
					if not obj:isLuck() and nil == obj:findInfectDriverExistBySkillId(infectSkillId) and true ~= ruleOutTags_[tostring(obj:getOTag())] then
						table.insert(result, obj)
					end
				end
			elseif ConfigSeekTargetRule.T_OBJ_FRIEND_PLAYER == ttype then
				for i = #BMediator:GetBData().sortPlayerObj.friend, 1, -1 do
					obj = BMediator:GetBData().sortPlayerObj.friend[i]
					if not obj:isLuck() and nil == obj:findInfectDriverExistBySkillId(infectSkillId) and true ~= ruleOutTags_[tostring(obj:getOTag())] then
						table.insert(result, obj)
					end
				end
		...
		elseif ConfigSeekTargetRule.T_OBJ_ATTACKER == ttype then
			...
			if nil ~= o then
				...
				local objs = nil
				if isEnemy then
					objs = BMediator:GetBData().sortBattleObjs.friend
				else
					objs = BMediator:GetBData().sortBattleObjs.enemy
				end
				for i = #objs, 1, -1 do
					obj = objs[i]
				...
				if o:isEnemy() == o:isEnemy(true) then
					...
					if o.attackDriver then
						...
						if nil ~= attackTargetTag then
							obj = BMediator:IsObjAliveByTag(attackTargetTag)
...
function BattleExpression.GetDeadFriendlyTargets(isEnemy, ttype, o, needSameTeam, ruleOutTags)
	...
	if ConfigSeekTargetRule.T_OBJ_SELF == ttype then
		...
		if ConfigSeekTargetRule.T_OBJ_ALL == ttype then
			...
			-- 所有单位
			for i = #BMediator:GetBData().sortDustObjs.friend, 1, -1 do
				obj = BMediator:GetBData().sortDustObjs.friend[i]
				if not obj:isLuck() and (not needSameTeam or oTeamIndex == obj:getObjectTeamIndex()) and true ~= ruleOutTags_[tostring(obj:getOTag())] then
					table.insert(result, obj)
				end
			end
			for i = #BMediator:GetBData().sortDustObjs.enemy, 1, -1 do
				obj = BMediator:GetBData().sortDustObjs.enemy[i]
				if not obj:isLuck() and (not needSameTeam or oTeamIndex == obj:getObjectTeamIndex()) and true ~= ruleOutTags_[tostring(obj:getOTag())] then
					table.insert(result, obj)
				end
			end
	...
	if ConfigSeekTargetRule.T_OBJ_SELF == ttype then
		...
		if ConfigSeekTargetRule.T_OBJ_ALL == ttype then
			...
			for i = #BMediator:GetBData().sortBattleObjs.friend, 1, -1 do
				obj = BMediator:GetBData().sortBattleObjs.friend[i]
				-- 排除木桩
				if not obj:isLuck() and not obj:isScarecrow() then
					if nil == obj:findInfectDriverExistBySkillId(infectSkillId) and true ~= ruleOutTags_[tostring(obj:getOTag())] then
						table.insert(result, obj)
					end
				end
			end
		...
		elseif ConfigSeekTargetRule.T_OBJ_ENEMY == ttype or ConfigSeekTargetRule.T_OBJ_FRIEND == ttype then
			...
			if ConfigSeekTargetRule.T_OBJ_ENEMY == ttype then
				for i = #BMediator:GetBData().sortBattleObjs.enemy, 1, -1 do
					obj = BMediator:GetBData().sortBattleObjs.enemy[i]
					-- 排除木桩
					if not obj:isLuck() and not obj:isScarecrow() and true ~= ruleOutTags_[tostring(obj:getOTag())] then
						if nil == obj:findInfectDriverExistBySkillId(infectSkillId) then
							table.insert(result, obj)
						end
					end
				end
...
@params result table
--]]
function BattleExpression.GetDeadFriendlyTargetsForPlayerSkill(isEnemy, ttype, o, ruleOutTags)
	local result = {}
	local ruleOutTags_ = ruleOutTags or {}
...
获取主角
--]]
function BattleExpression.GetFriendlyPlayerObj()
	return BMediator:GetBData().sortPlayerObj.friend
end
--[[
根据条件筛选符合条件的obj
...
function BattleExpression.SortTByRandom(targets)
	...
	local randomMark = BMediator:GetRandomManager():GetRandomInt(amount) - 1
	local index = 0
	for i = 1, amount do
		index = randomMark + i
