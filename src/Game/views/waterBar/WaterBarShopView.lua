...
local WaterBarShopView = class('WaterBarShopView', function()
    return ui.layer({name = 'Game.views.waterBar.WaterBarShopView', enableEvent = true})
end)
...
function WaterBarShopView:ctor()
    -- create view
    self.viewData_ = WaterBarShopView.CreateView()
...
function WaterBarShopView:onCleanup()
    VoProxy.EventUnbind(SHOP_PROXY_NAME, self.viewBindMap_, self)
end


function WaterBarShopView:getViewData()
...
function WaterBarShopView:onUpdateRefreshLeftSeconds_(signal)
...
function WaterBarShopView:onUpdateProductsGridView_(signal)
...
function WaterBarShopView:onUpdateProductsGridCell_(signal)
...
function WaterBarShopView:onUpdateProductCellHandler_(cellIndex, cellViewData, changedVoDefine)
    ...
    if changedVoDefine == nil or changedVoDefine == CELL_STRUCT.OPEN_LEVEL then
        ...
        cellViewData.lockLabel:setVisible(app.waterBarMgr:getBarLevel() < openLevel)
        display.commonLabelParams(cellViewData.lockLabel, {text = string.fmt(__('_num_级水吧解锁'), {_num_ = openLevel}), paddingH = 10})
    end
...
function WaterBarShopView.CreateView()
    local view = ui.layer()
    ...
    moneyBar:reloadMoneyBar({FOOD.GOODS.DEFINE.WATER_BAR_CURRENCY_ID}, false, {
    ...
    -- title bar
    local titleBar = ui.title({img = RES_DICT.TITLE_BAR}):updateLabel({fnt = FONT.D3, text = __('水吧商店'), paddingW = 60, safeW = 120, offset = cc.p(0,-2)})
