...
function BattleSuccessView:InitCommonLayer()
	...
	cb = function (sender)
		if self.canTouch then
			for i,v in ipairs(self.teamData) do
				...
				if cardId > 0 then
					CommonUtils.PlayCardSoundByCardId(cardId, SoundType.TYPE_UPGRADE_STAR, SoundChannel.BATTLE_RESULT, not BattleConfigUtils:UseJapanLocalize())
					break
				end
			end
	...
	elseif self.cleanCondition and self.trophyData.gradeConditionIds then
		...
		for i = 1, #allCleanIds do
			...
			local conditionLabelSize = display.getLabelContentSize(conditionLabel)
			if conditionLabelSize.width > 400 then
				display.commonLabelParams(conditionLabel, {reqW = 300, w = 400})
			else
				display.commonLabelParams(conditionLabel, {reqW = 300})
			end
			...
			for i,v in ipairs(cleanConditionNodes) do
				...
				local fireActionSeq = cc.Sequence:create(
					...
					cc.CallFunc:create(function ()
						...
						if nil ~= cleanedIds[tostring(cleanId)] then
							-- 播放满星音效
							PlayUIEffects(AUDIOS.UI.ui_war_assess.id)
						end
					end),
...
function BattleSuccessView:AddCommonBottomLayer(parentNode)
	...
	if ConfigBattleResultType.NO_EXP == self.viewType then
		...
		local ShowSelf = function (delayFrame)
			...
			if nil ~= repeatBtn then
				local rbtnActionSeq = cc.Sequence:create(
					cc.DelayTime:create(delayFrame / self.fps),
					cc.Show:create(),
					cc.FadeTo:create(0.5, 255))
				repeatBtn:runAction(rbtnActionSeq)
			end
		end
...
function BattleSuccessView:AddStairCurrencyLayer(parentNode)
	...
	if QuestBattleType.PVC == BMediator:GetOriBattleConstructData().questBattleType then
		stairCurrencyInfo = {
			{goodsId = PVC_POINT_ID, fieldName = 'integral', isFinalAmount = false},
			{goodsId = PVC_MEDAL_ID, fieldName = 'medal', isFinalAmount = true},
		}
	elseif QuestBattleType.UNION_BEAST == BMediator:GetOriBattleConstructData().questBattleType then
		stairCurrencyInfo = {
