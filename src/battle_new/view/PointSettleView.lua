...
function PointSettleView:InitPointLayer()
	...
	if true then
		local currentSummerPoint = checkint(self.trophyData.summerPoint or self.trophyData.point)
		local deltaSummerPoint = checkint(self.trophyData.deltaSummerPoint or self.trophyData.deltaPoint)
		local hasSummerPoint = currentSummerPoint - deltaSummerPoint
		if QuestBattleType.PT_DUNGEON == CommonUtils.GetQuestBattleByQuestId(checkint(self.trophyData.requestData.questId)) then
	...
	if nil ~= self.rewardsLayer then
		...
		skadaBtn:setPositionY(backBtn:getPositionY())
	end
	------------ 初始化动画状态 ------------
