...
邮箱界面
--]]
local MailView = class('MailView', function ()
	local node = CLayout:create(cc.size(890,586))
	node:setAnchorPoint(cc.p(0, 0))
	node.name = 'home.MailView'
...
function MailView:ctor(...)
	local function CreateView()
		...
		display.commonLabelParams(dialogue_tips,{text = __('当前暂无邮件通知'), fontSize = 24, color = '#4c4c4c'})
		...
		display.commonLabelParams(deleteAllBtn, fontWithColor(14, {text = __('删除已读' ) ,w = 120 , reqH = 50 , hAlign = display.TAC}))
		-- 领取全部
		local drawAllBtn = nil
		if isJapanSdk() then
			drawAllBtn = display.newButton(btnLayoutSize.width/2 + 76, 35, {n = _res('ui/common/common_btn_orange.png'), scale9 = true, size = cc.size(143, 62)})
			btnLayout:addChild(drawAllBtn, 3)
			display.commonLabelParams(drawAllBtn, fontWithColor(14, {text = __('一键领取'), hAlign = display.TAC}))
		else
			drawAllBtn = display.newButton(btnLayoutSize.width/2 + 76, 35, {n = _res('ui/common/common_btn_orange.png')})
			btnLayout:addChild(drawAllBtn, 3)
			display.commonLabelParams(drawAllBtn, fontWithColor(14, {text = __('一键领取'),w = 120 , reqH = 50 , hAlign = display.TAC}))
		end
		...
		local titleLabel = display.newLabel(10, rsize.height + 5,
			{reqW = 300 , text = '', ap = cc.p(0, 1), fontSize = 28, color = '#ba5c5c'})
		...
		local timeLabel = display.newLabel(rsize.width - 8, rsize.height - 46,
		{text = "",fontSize = 20, color = '#de5b3d', ap = cc.p(1.0, 0)})
		timeLabel:setTag(115)
		rightView:addChild(timeLabel)
		...
		-- 领取按钮
		local getBtn = display.newButton(0, 0, {n = _res('ui/common/common_btn_orange.png') , scale9 = true })
		display.commonUIParams(getBtn, {po = cc.p(rsize.width * 0.5, 30)})
		display.commonLabelParams(getBtn, fontWithColor(14,{text = __('领取') , paddingW = 20 }))
		rightView:addChild(getBtn)
		...
		if GAME_MODULE_OPEN.MAIL_COLLECTION then
			collectBtn = display.newCheckBox( rsize.width - 50 , 30 ,
	{ n = _res('ui/cards/love/card_ico_set_cover_normal.png') , s = _res('ui/cards/love/card_ico_set_cover_active.png')})
			rightView:addChild(collectBtn)
		end
		return {
			view             = view,
			emptyView        = emptyView,
