...
新鲜度恢复界面的修改
--]]
---@class VigourRecoveryView
local VigourRecoveryView = class('VigourRecoveryView', function()
    local node = CLayout:create(display.size)
    node.name = 'common.VigourRecoveryView'
...
function VigourRecoveryView:ctor( ... )
    self.args = unpack({...}) or {}
    self.moduleCallback = self.args.moduleCallback
    self.viewData = nil
    local function CreateView()
        local size = (cc.size(display.width, 560))
        ...
        -- 调整按钮
        local changeBtn = display.newButton(display.SAFE_L + 72, 500, {n = _res('ui/common/common_btn_orange.png'), tag = BTN_TAG.CHANGE_TEAM})
        ...
        -- 探索按钮
        local explorationBtn = require('common.CommonBattleButton').new({ pattern = 2,
            clickCallback = self.moduleCallback
        })
        explorationBtn:setName('BattleBTN')
        view:addChild(explorationBtn, 10 )
        explorationBtn:setTag(BTN_TAG.MOUDLE_GOTO)
        display.commonUIParams( explorationBtn,{po = cc.p(display.SAFE_R - 100, size.height - 150)})
        -- 队伍战斗力
        ...
        -- 切换按钮
        local switchBtnL = display.newButton(size.width / 2 - 448, size.height - 150, {n = _res('ui/common/battle_btn_fullstar.png'), tag = BTN_TAG.PRE_TEAM_BTN})
        switchBtnL:setScaleX(-1)
        view:addChild(switchBtnL, 10)
        local switchBtnR = display.newButton(size.width / 2 + 448, size.height - 150, {n = _res('ui/common/battle_btn_fullstar.png'), tag = BTN_TAG.NEXT_TEAM_BTM})
        ...
        local recoveryLabel = display.newLabel(size.width - 110, 150, fontWithColor(3, {text = __('快速回复')}))
        ...
        for i = 1, 3 do
            ...
            cc.CallFunc:create(function ()
                local arrow = display.newImageView(_res('ui/common/discovery_ico_arrow_up.png'), size.width/2, 290 - i*20)
        ...
        -- tips
        local tipsBtn = display.newButton(size.width/2 - 405, 504, {n = _res('ui/common/common_btn_tips.png'), tag = BTN_TAG.TIPS_BTN})
