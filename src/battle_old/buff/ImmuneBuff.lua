...
function ImmuneBuff:CauseEffect()
	...
	if nil ~= owner then
		...
		if ConfigBuffType.IMMUNE == btype then
			...
			owner:setDamageImmune(DamageType.PHYSICAL, true)

		elseif ConfigBuffType.IMMUNE_ATTACK_PHYSICAL == btype then

			owner:setDamageImmune(DamageType.ATTACK_PHYSICAL, true)
...
function ImmuneBuff:RecoverEffect()
	...
	if nil ~= owner then
		...
		if ConfigBuffType.IMMUNE == btype then
			...
			owner:setDamageImmune(DamageType.PHYSICAL, false)

		elseif ConfigBuffType.IMMUNE_ATTACK_PHYSICAL == btype then

			owner:setDamageImmune(DamageType.ATTACK_PHYSICAL, false)

		elseif ConfigBuffType.IMMUNE_SKILL_PHYSICAL == btype then

			owner:setDamageImmune(DamageType.SKILL_PHYSICAL, false)

		elseif ConfigBuffType.IMMUNE_ATTACK_HEAL == btype then

			owner:setDamageImmune(DamageType.ATTACK_HEAL, false)

		elseif ConfigBuffType.IMMUNE_SKILL_HEAL == btype then

			owner:setDamageImmune(DamageType.SKILL_HEAL, false)

		elseif ConfigBuffType.IMMUNE_HEAL == btype then

			owner:setDamageImmune(DamageType.HEAL, false)
