...
function UnionBeastBabyEatScene:createAct()
	local battleRootLayer = BMediator:GetBattleRoot()
	...
	local eposinbattleroot = BMediator:GetCellPosByRC(cellR, cellC)
	...
	-- 创建一个神兽幼崽spine
	local beastConfig = cardMgr.GetBeastConfig(self.beastId)
	local beastBabyId = checkint(beastConfig.petId)
	local beastBabyFormConfig = cardMgr.GetBeastBabyFormConfig(beastBabyId, self.energyLevel, 1)
...
function UnionBeastBabyEatScene:OnActEnter()
	if self.energyNode and self.spineNode then
		...
		local speed = BMediator:GetBConf().cellSizeWidth * 3
		...
		local spineNodeActionSeq = cc.Sequence:create(
			...
			cc.CallFunc:create(function ()
				...
				local beastConfig = cardMgr.GetBeastConfig(self.beastId)
				local beastBabyId = checkint(beastConfig.petId)
				local voiceConfig = cardMgr.GetUnionBeastBabyVoiceConfigByVoiceType(beastBabyId, UnionPetVoiceType.AFTER_ENERGY)
		...
		local soundActionSeq = cc.Sequence:create(
			...
			cc.CallFunc:create(function ()
				...
				if nil ~= beastBabyFormConfig then					
					...
					PlayUIEffects(effectId)
				end
			end),
			cc.DelayTime:create(moveTime),
			-- 吃音效
			cc.CallFunc:create(function ()
				PlayUIEffects(AUDIOS.UI.ui_shoutuanzi_energy.id)
			end),
			cc.DelayTime:create(animationsData['eat'].duration + 0.1),
			-- 翻滚音效
			cc.CallFunc:create(function ()
				PlayUIEffects(AUDIOS.UI.ui_shoutuanzi_win.id)
			end),
...
function UnionBeastBabyEatScene:clearAll()
	-- 所有物体强制隐藏
	for k,v in pairs(BMediator:GetBData().battleObjs) do
		v:forceHide()
	end

	-- 隐藏所有ui
	BMediator:GetViewComponent():ShowUILayer(false)
end
