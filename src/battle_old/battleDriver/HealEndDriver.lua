...
function HealEndDriver:Init()
	...
	local waveInfo = self:GetOwner():GetBData():getEnemyMembers()[self:GetWave()]
...
function HealEndDriver:CanDoLogic()
	...
	elseif 0 == #bdata.sortBattleObjs.friend then
		...
		for i = #bdata.sortDustObjs.friend, 1, -1 do
			local o = bdata.sortDustObjs.friend[i]
			local cardId = o:getOCardId()
			local targetInfo = self:GetTargetInfoByIdAndCampType(cardId, ConfigCampType.FRIEND)

			if nil ~= targetInfo and self:GetWave() == o:getObjectWave() then
				if bdata:canBuyRevival() then
					-- 可以买活
		...
		for i = #bdata.sortBattleObjs.friend, 1, -1 do
			local o = bdata.sortBattleObjs.friend[i]
			local cardId = o:getOCardId()
			local targetInfo = self:GetTargetInfoByIdAndCampType(cardId, ConfigCampType.FRIEND)

			if nil ~= targetInfo and self:GetWave() == o:getObjectWave() then
				-- 是目标物体
				if targetInfo.targetHpPercent > o:getMainProperty():getCurHpPercent() then
		...
		for i = #bdata.sortBattleObjs.enemy, 1, -1 do
			local o = bdata.sortBattleObjs.enemy[i]
			local cardId = o:getOCardId()
		...
		for i = #bdata.sortRestObjs, 1, -1 do
			local o = bdata.sortRestObjs[i]
			local cardId = o:getOCardId()
			local isEnemy = o:isEnemy(true)
