...
function ConnectButton:ctor( ... )
	...
	self.cardHeadPath = args.cardHeadPath
...
function ConnectButton:InitUI()
	local function CreateView()
		...
		local headIconPath = self.cardHeadPath
...
@params energyPercent number obj 能量百分比
@params canAct bool obj是否可以行动
@params state OState obj 状态
@params inAbnormalState bool 是否处于无法释放连携技的状态
--]]
function ConnectButton:RefreshButton(energyPercent, canAct, state, inAbnormalState)
	self:RefreshButtonByEnergy(energyPercent)
	self:RefreshButtonByState(canAct, state, inAbnormalState)
end
--[[
刷新按钮 单能量
@params energyPercent number obj 能量百分比
--]]
function ConnectButton:RefreshButtonByEnergy(energyPercent)
	self.viewData.cardEnergyBar:setValue(energyPercent * MAX_ENERGY)
...
@params inAbnormalState bool 是否处于无法释放连携技的状态
--]]
function ConnectButton:RefreshButtonByState(canAct, state, inAbnormalState)
	if not self:GetCanUse() then
		-- 有连携对象死亡
		self:DisableConnectButton()
		return 
	end
	if not canAct or inAbnormalState then
		-- 无法行动
...
移除自己
--]]
function ConnectButton:RemoveSelf()
