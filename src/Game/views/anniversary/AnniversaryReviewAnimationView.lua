...
local AnniversaryReviewAnimationView = class('AnniversaryReviewAnimationView', function()
    return display.newLayer(0, 0, {name = 'Game.views.anniversary.AnniversaryReviewAnimationView'})
end)

local RES_DICT = {
    SKIP_BTN = app.anniversaryMgr:GetResPath('arts/stage/ui/opera_btn_skip.png'),
    BG_IMAGE = app.anniversaryMgr:GetResPath('ui/cards/marry/card_contract_bg_memory.jpg'),
}
...
function AnniversaryReviewAnimationView:ctor(args)
...
CreateView = function() 
    ...
    display.commonLabelParams(skipBtn, fontWithColor(20, {text = app.anniversaryMgr:GetPoText(__('跳过')), fontSize = 24, ap = display.RIGHT_CENTER, offset = skipOffP}))
...
CreatePage = function(pageName)
    ...
    local pageFile = app.anniversaryMgr:GetResPath(string.format('ui/anniversary/reviewAnimate/%s.csb', tostring(pageName)))
...
-------------------------------------------------
-- get / set
function AnniversaryReviewAnimationView:getViewData()
...
function AnniversaryReviewAnimationView:close()
    -- clean page
    self:cleanPage()
...
function AnniversaryReviewAnimationView:cleanPage()
    -- stop listen
...
function AnniversaryReviewAnimationView:playPage(pageName)
    -- clean page
    self:cleanPage()
    ...
    if self.pageViewData_.timeline then
        local onFrameEvent = function(frame)
            ...
            if string.sub(frameName, 0, BGM_NAME_LENGTH) == BGM_NAME_PREFIX then
                ...
                local audioPath = app.anniversaryMgr:GetResPath(string.fmt('ui/anniversary/reviewAnimate/mp3/%1.mp3', frameName))
        ...
        self:eachPageNode_(self.pageViewData_.pageView, function(node)
            ...
            elseif string.sub(nodeName, 0, SPINE_NAME_LENGTH) == SPINE_NAME_PREFIX then
                local spinePath = app.anniversaryMgr:GetSpinePath(string.fmt('ui/anniversary/reviewAnimate/spine/%1', nodeName))
...
function AnniversaryReviewAnimationView:playAudio_(audioPath)
...
function AnniversaryReviewAnimationView:stopAudio_()
    cc.SimpleAudioEngine:getInstance():stopMusic(true)
end


function AnniversaryReviewAnimationView:hideSkip_(isFast)
...
function AnniversaryReviewAnimationView:showSkip_(isFast, isForever)
    self.isControllable_ = false
    local finishCB = function()
        self.isControllable_ = true
...
function AnniversaryReviewAnimationView:eachPageNode_(pageView, eachFunc)
...
function AnniversaryReviewAnimationView:startCheckPlayEnd_()
...
function AnniversaryReviewAnimationView:stopCheckPlayEnd_()
...
function AnniversaryReviewAnimationView:onClickSkipHotspotHandler_(sender)
...
function AnniversaryReviewAnimationView:onClickCloseButtonHandler_(sender)
...
return AnniversaryReviewAnimationView
