...
---@class KofArenaRankMediator :Mediator
local KofArenaRankMediator = class("KofArenaRankMediator", Mediator)
local NAME = "KofArenaRankMediator"
---@type UIManager
...
function KofArenaRankMediator:ctor( viewComponent )
...
function KofArenaRankMediator:InterestSignals()
    local signals = {
        POST.RANK_KOF_ARE_NARANK.sglName 
    }
    return signals
end

function KofArenaRankMediator:ProcessSignal( signal )
    local body = signal:GetBody()
    local name = signal:GetName()
    if name == POST.RANK_KOF_ARE_NARANK.sglName  then
        self.rankData  = body
        self.leftSecond = body.kofArenaRankLeftSeconds
...
--[[
    获取到排行数据
--]]
function KofArenaRankMediator:GetRankData()
    return self.rankData
end
--[[
    获取到上周的排行数据
--]]
function KofArenaRankMediator:GetLastRankData()
    local rankData = self.rankData
    return rankData.lastKofArenaRank
end
--[[
    获取到当前所属的单元
--]]
function KofArenaRankMediator:GetMyKofArenaSegment()
    local rankData = self:GetRankData() or {}
    local myKofArenaRank = checkint(rankData.myKofArenaSegment)
    return myKofArenaRank

end

function KofArenaRankMediator:Initial( key )
    self.super.Initial(self, key)
    ---@type KofArenaRankScene
    local viewComponent = require("Game.views.KofArenaRankScene").new()
...
--[[
    更新倒计时时间的显示
--]]
function KofArenaRankMediator:UpdateLeftTimeLabel()
...
--[[
    更新自己的成绩显示
--]]
function KofArenaRankMediator:UpdateMySelfLayout()
    ...
    if arenaSegment ~= 0 and ( checkint(self.selectIndex) == arenaSegment ) then
        ...
        local myKofArenaRank    = checkint(rankData.myKofArenaRank)
        local myKofArenaScore   = checkint(rankData.myKofArenaScore)
        ...
        display.commonLabelParams(winTimesLabel , {text = string.format(__('进攻胜场 %d')  , myKofArenaScore),reqW = 155 } )
...
插入listview 的子项
--]]
function KofArenaRankMediator:InsertListViewCell()
    local levelSegmentConfig = CommonUtils.GetConfigAllMess('levelSegment' , 'kofArena')
    ...
    for ii, vv in pairs(levelKeys) do
        ...
        display.commonLabelParams(btn, fontWithColor('14' , {text = v.name , reqW  =  150 }))
...
function KofArenaRankMediator:ListCellRunAction()
    local viewData = self.viewComponent.viewData
    local listView  = viewData.listView
...
--[[
    添加自己所属cell 的标志
--]]
function KofArenaRankMediator:AddMeInCellFlags(myKofArenaSegment)
    local viewData = self.viewComponent.viewData
    local listView  = viewData.listView
...
function KofArenaRankMediator:ButtonAction(sender)
...
--[[
    查看上周的排行榜
--]]
function KofArenaRankMediator:LookLastWeekRank()
    ...
    local LobbyLastRankingView  = require( 'Game.views.LobbyLastRankingView' ).new({tag = 1100, lastRank = lastRankData, iconStr = __('场'), title = __('上周天城演武排行榜')})
...
--[[
    查看排行奖励
--]]
function KofArenaRankMediator:LookRankReward()
...
function KofArenaRankMediator:SwitchBtnClick(sender)
    PlayAudioByClickNormal()
    local tag = sender:getTag()
...
--[[
    获取当前周的当前的排行榜奖励
--]]
function KofArenaRankMediator:GetRankOneDatabyId(stageId)
    local rankData = self:GetRankData()  or {}
    local kofArenaRank =rankData.kofArenaRank or {}
...
--[[
    获取上一周的当前的排行榜奖励
--]]
function KofArenaRankMediator:GetLastRankOneDatabyId(stageId)
    local rankData = self:GetRankData()  or {}
    local kofArenaRank =rankData.lastKofArenaRank or {}
    local oneKofArenaRank = kofArenaRank[tostring(stageId)] or {}
    return oneKofArenaRank
end
...
--[[
    倒计时
--]]
function KofArenaRankMediator:CountDownTimes()
...
function KofArenaRankMediator:GetFightTeamCaptainByFightTeam(fightTeam)
    local captainTable = {}
...
--[[
    切换到其他阶段的排行榜
--]]
function KofArenaRankMediator:SwitchOtherStageById(stageId)
    local viewData = self.viewComponent.viewData
    local gridView = viewData.gridView
...
function KofArenaRankMediator:OnDataSource(cell, idx)
    local pCell = cell
    local index = idx +1
    ...
    xTry(function()
        ...
        display.commonLabelParams(pCell.winLabel , {reqW = 155, text = string.format(__('进攻胜场 %d') , checkint(datas.winTimes))})
...
function KofArenaRankMediator:CreateShowFightTeam(teamData, teamId , targetNode )
...
function KofArenaRankMediator:EnterLayer(  )
    self:SendSignal(POST.RANK_KOF_ARE_NARANK.cmdName ,{})
end

function KofArenaRankMediator:OnRegist(  )
    regPost(POST.RANK_KOF_ARE_NARANK)
    self:EnterLayer()
end

function KofArenaRankMediator:OnUnRegist(  )
    unregPost(POST.RANK_KOF_ARE_NARANK)
