--[[
 * author : liuzhipeng
 * descpt : 活动 20春活 积分奖励Mediator
--]]
local Mediator = mvc.Mediator
local SpringActivity20PointRewardsMediator = class("SpringActivity20PointRewardsMediator", Mediator)
local NAME = "Game.mediator.springActivity20.SpringActivity20PointRewardsMediator"
SpringActivity20PointRewardsMediator.NAME = NAME

function SpringActivity20PointRewardsMediator:ctor(params, viewComponent)
    self.super.ctor(self, NAME, viewComponent)
    self.ctorArgs_ = {homeData = app.springActivity20Mgr:GetHomeData()}
end

-------------------------------------------------
-- inheritance method
function SpringActivity20PointRewardsMediator:Initial(key)
    ...
    -- create view
    local viewComponent = require('Game.views.springActivity20.SpringActivity20PointRewardsView').new({mediatorName = NAME})
...
function SpringActivity20PointRewardsMediator:InitData_(homeData)
    local datas = {}
    local curPoint = nil
    local curPlot = 0

    ------------------------- 初始化剧情列表数据 -------------------------
    local serStory = homeData.unlockStoryMap or {}
    local serUnlockStory = serStory

    local plotConf = CommonUtils.GetConfigAllMess('pointStory', 'springActivity2020') or {}
    local plotDatas = {}
    if next(plotConf) ~= nil then
        local plotStaeIndexs = {}
        for i, plotData in orderedPairs(plotConf) do
            local goodsId  = checkint(app.springActivity20Mgr:GetPointGoodsId())
            local goodsNum = checkint(plotData.num)
            if curPoint == nil then
                curPoint = app.springActivity20Mgr:GetPointAmount()
            end
...
--==============================--
function SpringActivity20PointRewardsMediator:InitPlotState(targetNum, curPoint)
    local state = 0
...
--==============================--
function SpringActivity20PointRewardsMediator:InitPlotRewardDrawState(plotPointRewards, rewardsId, consumeNum, curPoint)
    local state = 1
...
function SpringActivity20PointRewardsMediator:InitView_()
...
function SpringActivity20PointRewardsMediator:InitOwnerScene_()
    self.ownerScene_ = app.uiMgr:GetCurrentScene()
end

function SpringActivity20PointRewardsMediator:cleanupView()
    local viewComponent = self:GetViewComponent()
...
function SpringActivity20PointRewardsMediator:OnRegist()
    regPost(POST.SPRING_ACTIVITY_20_DRAW_POINT_REWARDS)
    regPost(POST.SPRING_ACTIVITY_20_UNLOCK_STORY)
    self:EnterLayer()
end
function SpringActivity20PointRewardsMediator:OnUnRegist()
    unregPost(POST.SPRING_ACTIVITY_20_DRAW_POINT_REWARDS)
    unregPost(POST.SPRING_ACTIVITY_20_UNLOCK_STORY)
    self:cleanupView()
end


function SpringActivity20PointRewardsMediator:InterestSignals()
    return {
        POST.SPRING_ACTIVITY_20_DRAW_POINT_REWARDS.sglName,
        POST.SPRING_ACTIVITY_20_UNLOCK_STORY.sglName
    }
end

function SpringActivity20PointRewardsMediator:ProcessSignal(signal)
    local name = signal:GetName()
    local body = signal:GetBody() or {}

    if name == POST.SPRING_ACTIVITY_20_DRAW_POINT_REWARDS.sglName then
        -- show reward
    ...
    elseif POST.SPRING_ACTIVITY_20_UNLOCK_STORY.sglName == name then
        local requestData = body.requestData or {}
...
function SpringActivity20PointRewardsMediator:GetViewData()
    return self.viewData_
end

function SpringActivity20PointRewardsMediator:GetOwnerScene()
    return self.ownerScene_
end

-------------------------------------------------
-- public method
function SpringActivity20PointRewardsMediator:EnterLayer()
end

function SpringActivity20PointRewardsMediator:RefreshUI()
    local viewComponent = self:GetViewComponent()
    -- viewComponent:InitPlotListView(self.plotDatas)
...
function SpringActivity20PointRewardsMediator:SortPlotRewards(plotRewards)
...
function SpringActivity20PointRewardsMediator:OnDataSourceAdapter(p_convertview, idx)
...
function SpringActivity20PointRewardsMediator:OnClicEnterPlotAction(sender)
    ...
    if state == 0 then
        local storyCollectConf = CommonUtils.GetConfigAllMess('storyCollect', 'springActivity2020') or {}
        local storyCollect = storyCollectConf[tostring(storyId)] or {}
        local scene = app.uiMgr:GetCurrentScene()
        local CommonTip  = require( 'common.NewCommonTip' ).new({text = tostring(storyCollect.name), extra = app.springActivity20Mgr:GetPoText(__('解锁需要收集:')),
        richtext = {
    ...
    local stage = require( "Frame.Opera.OperaStage" ).new({
        ...
        if not self.serUnlockStory[tostring(storyId)] then
            self:SendSignal(POST.SPRING_ACTIVITY_20_UNLOCK_STORY.cmdName , {
                storyId = storyId,
                index = index,
            })
            -- app:DispatchObservers(POST.SPRING_ACTIVITY_20_UNLOCK_STORY.sglName, {requestData = {storyId = storyId, index = index}})
        end
...
function SpringActivity20PointRewardsMediator:OnClickDrawBtnAction(sender)
    ...
    if state == 1 then
        ...
        app.uiMgr:ShowInformationTips(string.format(app.springActivity20Mgr:GetPoText(__('当前%s数量不足')), tostring(goodsConfig.name)))
        return 
    elseif state == 3 then
        app.uiMgr:ShowInformationTips(app.springActivity20Mgr:GetPoText(__('已领取')))
        return 
    end

    self:SendSignal(POST.SPRING_ACTIVITY_20_DRAW_POINT_REWARDS.cmdName, {rewardId = data.rewardsId, index = index})
    -- app:DispatchObservers(POST.SPRING_ACTIVITY_20_DRAW_POINT_REWARDS.sglName, {rewards = {{goodsId = 151066, num = 11}}, requestData = {rewardId = data.rewardsId, index = index}})
end


return SpringActivity20PointRewardsMediator
