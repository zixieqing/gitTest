	...
	cb = function (sender)
		if self.canTouch then
			for i,v in ipairs(self.teamData) do
				...
				if cardId > 0 then
					CommonUtils.PlayCardSoundByCardId(cardId, SoundType.TYPE_BATTLE_SUCCESS, SoundChannel.BATTLE_RESULT)
					break
				end
			end
...
function ShareBossSuccessView:InitCommonDamageLayer()
	...
	display.commonLabelParams(skadaBtn, fontWithColor('14', {text = __('伤害统计') , paddingW = 20 }))
