...
function LobbyShopViewMediator:Initial( key )
	...
	local viewComponent  = require( 'Game.views.stores.LobbyShopView' ).new()
...
--更新数量ui值
function LobbyShopViewMediator:UpdateCountUI()
...
function LobbyShopViewMediator:onClickBatchBuyButtonHandler_(sender)
...
function LobbyShopViewMediator:CellButtonAction(sender)
	...
	if CommonUtils.CheckIsOwnSkinById(data.goodsId) then 
		uiMgr:ShowInformationTips('已拥有该皮肤') 
		return 
	end
