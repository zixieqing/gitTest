...
function KOFShopViewMediator:ProcessSignal(signal )
	...
	if name == SIGNALNAMES.Restaurant_Shop_Home_Callback then
		self.shopData = checktable(body.kofArena)
		local data = {
		...
		self:GetViewComponent():InitShowUiAndTopUi(data)
...
function KOFShopViewMediator:Initial( key )
	...
	local viewComponent  = require( 'Game.views.stores.KOFShopView' ).new()
...
--更新数量ui值
function KOFShopViewMediator:UpdateCountUI()
...
function KOFShopViewMediator:onClickBatchBuyButtonHandler_(sender)
