...
CardPropertyConstructStruct = {
	...
	--]]
	New = function (
			cardId, level, breakLevel, favorLevel, petData, talentData, bookData, catGeneData,
...
ConfigBuffInfoStruct = {
	...
	@params triggerInsideCD number buff触发的内置间隔
...
ObjectSendBulletData = {
	--[[
	------------ 基本信息 ------------
	@params tag int 子弹唯一标识
	@params oname string 子弹名字
	@params otype ConfigEffectBulletType 特效子弹类型
	@params causeType ConfigEffectCauseType 特效指向类型
	@params ownerTag int 发射者tag
	@params ownerViewModelTag int 发射者的展示层tag
	@params targetTag int 目标tag 如果发射的特效为集团性效果则为空
	@params targetViewModelTag int 目标的展示层tag
	@params targetDead bool 目标是否死亡
	------------ spine动画信息 ------------
	@params spineId string 特效id 缓存中的id
	@params actionName string spine动画的动作名
	@params bulletZOrder int 特效层级
	@params oriLocation cc.p 初始位置
	@params targetLocation cc.p 初始目标位置
	@params bulletScale number 特效缩放
	@params fixedPos cc.p 修正位置
	@params towards bool 朝向 是否朝向右
	@params shouldShakeWorld bool 是否需要造成震屏
	@params needHighlight bool 是否需要高亮
	------------ 数据信息 ------------
	@params damageData ObjectDamageStruct 伤害信息
	@params durationTime number 持续时间
	@params causeEffectCallback function 收到自定义事件的回调函数 
	--]]
	New = function (
			tag, oname, otype, causeType, ownerTag, ownerViewModelTag, targetTag, targetViewModelTag, targetDead,
		...
		this.ownerViewModelTag = nil 			-- owner的展示层模型tag
		this.targetTag = nil 					-- 目标tag
		this.targetViewModelTag = nil 			-- target展示层模型tag
	...
	Init = function (self,
		...
		self.ownerViewModelTag = ownerViewModelTag
		self.targetTag = targetTag
		self.targetViewModelTag = targetViewModelTag
...
---------------------------------------------------
-- 创建一个bullet view需要的构造数据结构 --
---------------------------------------------------
BulletViewConstructorStruct = {
	--[[
	@params otype ConfigEffectBulletType 特效子弹类型
	@params causeType ConfigEffectCauseType 特效指向类型
	@params effectId int 特效的id
	@params bulletScale number 子弹的外部缩放
	@params towards BattleObjTowards 初始的朝向
	--]]
	New = function (otype, causeType, effectId, bulletScale, towards)
		local this = NewStruct(BulletViewConstructorStruct, BaseStruct)

		--[[---------- 初始化数据结构 ------------
		this.otype = ConfigEffectBulletType.BASE 		-- 特效子弹类型
		this.causeType = ConfigEffectCauseType.BASE 	-- 特效指向类型
		this.effectId = nil 					-- 特效的id
		this.bulletScale = 1 					-- 子弹的外部缩放
		this.towards = BattleObjTowards.BASE 	-- 初始的朝向
		------------ 初始化数据结构 ----------]]--

		this:Init(otype, causeType, effectId, bulletScale, towards)

		return this
	end,
	Init = function (self, otype, causeType, effectId, bulletScale, towards)
		self.otype = otype
		self.causeType = causeType
...
ObjectViewModelConstructorStruct = {
	--[[
	@params tag int 展示层tag
	@params logicModelTag int 对应的逻辑层tag
	@params avatarScale number 缩放
	@params spineData ObjectSpineDataStruct spine信息
	--]]
	New = function (tag, logicModelTag, avatarScale, spineData)
		local this = NewStruct(ObjectViewModelConstructorStruct, BaseStruct)

		--[[---------- 初始化数据结构 ------------
		this.tag = nil 							-- 展示层tag
		this.logicModelTag = nil 				-- 对应的逻辑层tag
		this.avatarScale = nil 					-- 卡牌avatar缩放比
		this.spineData = nil 					-- spine信息
		------------ 初始化数据结构 ----------]]--

		this:Init(tag, logicModelTag, avatarScale, spineData)

		return this
	end,
	Init = function (self,
			tag, logicModelTag, avatarScale, spineData
		)

		self.tag = tag
		self.logicModelTag = logicModelTag
		self.avatarScale = avatarScale
...
ObjectIdStruct = {
	--[[
	@params tag int 辨识物体唯一性的tag
	@params elementType BattleElementType 战斗元素类型
	--]]
	New = function (tag, elementType)
		local this = NewStruct(ObjectIdStruct, BaseStruct)

		--[[---------- 初始化数据结构 ------------
		this.tag = nil 							-- 卡牌唯一标识
		this.elementType = nil 					-- 战斗元素类型
		------------ 初始化数据结构 ----------]]--

		this:Init(tag, elementType)

		return this
	end,
	Init = function (self, tag, elementType)
...
逻辑层模型的构造数据
--]]
ObjectLogicModelConstructorStruct = {
	--[[
	@params idInfo ObjectIdStruct 描述物体唯一标志的数据
	@params objInfo ObjectConstructorStruct ... 描述物体特征的构造数据
	--]]
	New = function (idInfo, objInfo)
		local this = NewStruct(ObjectLogicModelConstructorStruct, BaseStruct)

		--[[---------- 初始化数据结构 ------------
		this.idInfo = nil 						-- 卡牌唯一标识
		this.objInfo = nil 						-- 战斗元素类型
		------------ 初始化数据结构 ----------]]--

		this:Init(idInfo, objInfo)

		return this
	end,
	Init = function (self, idInfo, objInfo)
		self.idInfo = idInfo
...
刷新渲染层transform需要的数据
--]]
ObjectRenderRefreshTranformStruct = {
	--[[
	@params x number x坐标
	@params y number y坐标
	...
	--]]
	New = function (x, y, towards, zorder)
		local this = NewStruct(ObjectRenderRefreshTranformStruct, BaseStruct)

		--[[---------- 初始化数据结构 ------------
		this.x = 0 								-- x坐标
		this.y = 0 								-- y坐标
		this.towards = nil 						-- 朝向
		this.zorder = nil 						-- cocos2dx zorder
		------------ 初始化数据结构 ----------]]--

		this:Init(x, y, towards, zorder)

		return this
	end,
	Init = function (self, x, y, towards, zorder)
		self.x = x
		self.y = y
		self.towards = towards
		self.zorder = zorder
...
刷新渲染层state需要的数据
--]]
ObjectRenderRefreshStateStruct = {
	--[[
	@params hpPercent number 血量百分比
	@params energyPercent number 能量百分比
	--]]
	New = function (hpPercent, energyPercent)
		local this = NewStruct(ObjectRenderRefreshStateStruct, BaseStruct)

		--[[---------- 初始化数据结构 ------------
		this.hpPercent = nil 					-- 坐标信息
		this.energyPercent = nil 				-- 朝向
		------------ 初始化数据结构 ----------]]--

		this:Init(hpPercent, energyPercent)

		return this
	end,
	Init = function (self, hpPercent, energyPercent)
...
qte buff 点击物体构造数据
--]]
QTEAttachObjectConstructStruct = {
	--[[
	@params attachType QTEAttachObjectType qte物体类型
	@params skillId int 技能id
	@params ownerTag int 被施法者tag
	@params casterTag int 施法者tag
	@params maxTouch int 最大点击次数
	@params qteBuffs list<map<{qteTapTime, buffType}>>
	--]]
	New = function (attachType, skillId, ownerTag, casterTag, maxTouch, qteBuffs)
		local this = NewStruct(QTEAttachObjectConstructStruct, BaseStruct)

		--[[---------- 初始化数据结构 ------------
		this.attachType = QTEAttachObjectType.BASE 	-- qte物体类型
		this.skillId = nil 						-- 技能id
		this.ownerTag = nil 					-- 被施法者tag
		this.casterTag = nil 					-- 施法者tag
		this.maxTouch = nil 					-- 最大点击次数
		this.qteBuffs = nil 					-- qte包含的buff信息
		------------ 初始化数据结构 ----------]]--

		this:Init(attachType, skillId, ownerTag, casterTag, maxTouch, qteBuffs)

		return this
	end,
	Init = function (self,
			attachType, skillId, ownerTag, casterTag, maxTouch, qteBuffs
		)
		...
		self.ownerTag = ownerTag
		self.casterTag = casterTag
		self.maxTouch = maxTouch or 0
		self.qteBuffs = qteBuffs or {}
	...
	@params buffType ConfigBuffType buff类型
	--]]
	AddAQTEBuffInfo = function (self, qteTapTime, buffType)
		table.insert(self.qteBuffs, 1, {qteTapTime = qteTapTime, buffType = buffType})
	end,
	--[[
	设置最大点击次数
	@params tapTime int 点击次数
	--]]
	SetMaxTouch = function (self, tapTime)
		if self.maxTouch < tapTime then
			self.maxTouch = tapTime
		end
	end
}
------------------------------------------------------------------------------------------------------
