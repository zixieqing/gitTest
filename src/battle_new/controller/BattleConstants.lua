...
ObjPP = {
	...
	CAUSE_DAMAGE_PHYSICAL 		= 24, 						-- 所有伤害增加 乘法系数 最终系数
	------------ 伤害相关的系数 ------------
	------------ 治疗相关的系数 ------------
	GET_HEAL_ATTACK 			= 25, 						-- 受到的平a治疗增加 乘法系数 最终系数
	GET_HEAL_SKILL 				= 26, 						-- 受到的技能治疗增加 乘法系数 最终系数
	GET_HEAL_ALL 				= 27, 						-- 受到的所有治疗增加 乘法系数 最终系数
	CAUSE_HEAL_ATTACK 			= 28, 						-- 造成的平a治疗增加 乘法系数 最终系数
	CAUSE_HEAL_SKILL 			= 29, 						-- 造成的技能治疗增加 乘法系数 最终系数
	CAUSE_HEAL_ALL 				= 30, 						-- 造成的所有治疗增加 乘法系数 最终系数
	------------ 治疗相关的系数 ------------
...
ConfigBuffType = {
	...
	ISD 					= 15,	--造成X点伤害 这条会有三个参数 造成[1] * 攻击力 + [2 固定值] [3 是否套用其他系数 1 不套用 其他值 套用]
	...
	VIEW_TRANSFORM 			= 131,  --变形buff >>>由目标卡牌的[1]皮肤id(-1为任意皮肤id)做[2]动作 变形为[3]皮肤id(此处不能为-1)变形后皮肤做[4]动作进行衔接
...
ConfigBattleResultType = {
	...
	ONLY_RESULT_AND_REWARDS = 10, -- 只有战斗结果+掉落
	REPLAY                  = 11, -- 回放战斗
}
...
BattleTags = {
	...
	------------ battle obj ------------
	BT_FRIEND 				= 1, 			-- 友方obj
	BT_CONFIG_ENEMY 		= 2, 			-- 敌方obj
	BT_OTHER_ENEMY 			= 3, 			-- 非qte召唤物
	BT_BECKON 				= 4, 			-- qte召唤物
	BT_WEATHER 				= 5, 			-- 天气物体
	BT_FRIEND_PLAYER		= 6, 			-- 友方主角
	BT_ENEMY_PLAYER 		= 7, 			-- 敌方主角
	BT_OBSERVER 			= 8, 			-- 引导ob
	BT_GLOBAL_EFFECT 		= 9, 			-- 全局效果obj
	BT_BULLET 				= 10, 			-- 子弹
	------------ logic model ------------
	BT_ATTACK_MODIFIER 		= 101, 			-- 攻击特效
	BT_TRIGGER 				= 102, 			-- 触发器
	BT_CI_SCENE 			= 103, 			-- ci场景
	BT_QTE_ATTACH 			= 104, 			-- qte冰
	------------ view model ------------
	BT_VIEW_MODEL 			= 201, 			-- view model tag
	------------ other ------------
	BT_DEBUFF 				= 2000 			-- debuff tag
}

-- 物体tag 两边开区间
BattleObjTagConfig = {
	------------ battle obj ------------
	[BattleTags.BT_FRIEND] 				= {lower = 0, 			upper = 1000, 			elementType = BattleElementType.BET_CARD}, 		-- 友方obj
	[BattleTags.BT_CONFIG_ENEMY] 		= {lower = 1000, 		upper = 2000, 			elementType = BattleElementType.BET_CARD}, 		-- 敌方obj
	[BattleTags.BT_OTHER_ENEMY] 		= {lower = 2000, 		upper = 3000, 			elementType = BattleElementType.BET_CARD}, 		-- 非qte召唤物
	[BattleTags.BT_BECKON] 				= {lower = 3000, 		upper = 4000, 			elementType = BattleElementType.BET_CARD}, 		-- qte召唤物
	[BattleTags.BT_WEATHER] 			= {lower = 4000, 		upper = 5000, 			elementType = BattleElementType.BET_WEATHER}, 	-- 天气物体
	[BattleTags.BT_FRIEND_PLAYER] 		= {lower = 5000, 		upper = 6000, 			elementType = BattleElementType.BET_PLAYER}, 	-- 友方主角
	[BattleTags.BT_ENEMY_PLAYER] 		= {lower = 6000, 		upper = 7000, 			elementType = BattleElementType.BET_PLAYER}, 	-- 敌方主角
	[BattleTags.BT_OBSERVER] 			= {lower = 7000, 		upper = 8000, 			elementType = BattleElementType.BET_OB}, 		-- 引导ob
	[BattleTags.BT_GLOBAL_EFFECT] 		= {lower = 8000, 		upper = 8100, 			elementType = BattleElementType.BET_OB}, 		-- 全局效果obj
	[BattleTags.BT_BULLET] 				= {lower = 10000, 		upper = nil, 			elementType = BattleElementType.BET_BULLET}, 	-- 子弹
	------------ logic model ------------
	[BattleTags.BT_ATTACK_MODIFIER] 	= {lower = 0, 			upper = nil, 			elementType = BattleElementType.BET_BASE, 		ignore = true}, 		-- 攻击特效
	[BattleTags.BT_TRIGGER] 			= {lower = 0, 			upper = nil, 			elementType = BattleElementType.BET_BASE, 		ignore = true}, 		-- 触发器
	[BattleTags.BT_CI_SCENE] 			= {lower = 0, 			upper = nil, 			elementType = BattleElementType.BET_BASE, 		ignore = true}, 		-- ci场景
	[BattleTags.BT_QTE_ATTACH] 			= {lower = 0, 			upper = nil, 			elementType = BattleElementType.BET_BASE, 		ignore = true}, 		-- qte冰
	------------ view model ------------
	[BattleTags.BT_VIEW_MODEL] 			= {lower = 100, 		upper = nil, 			elementType = BattleElementType.BET_BASE, 		ignore = true} 			-- view mode
}
