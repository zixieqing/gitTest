...
兑换列表cell
--]]
local ActivitySinglePayCell = class('ActivitySinglePayCell', function ()
	local ActivitySinglePayCell = CGridViewCell:new()
	ActivitySinglePayCell.name = 'home.ActivitySinglePayCell'
	ActivitySinglePayCell:enableNodeEvents()
	return ActivitySinglePayCell
end)

function ActivitySinglePayCell:ctor( params )
    ...
    -- 描述
    self.descrLabel = display.newLabel(20, bgSize.height - 16, fontWithColor(16, {text = '', ap = cc.p(0, 0.5)}))
    eventNode:addChild(self.descrLabel, 5)
    -- 领取次数
    self.timeLabel = display.newLabel(bgSize.width - 20, bgSize.height - 16, fontWithColor(16, {text = '', ap = cc.p(1, 0.5)}))
