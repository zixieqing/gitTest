--[[
 * author : liuzhipeng
 * descpt : 抽卡 常驻皮肤卡池 十连动画View 
--]]
local CapsuleBasicSkinAnimationTenView = class('CapsuleBasicSkinAnimationTenView', function ()
    local node = CLayout:create(display.size)
    node.name = 'home.CapsuleBasicSkinAnimationTenView'
...
function CapsuleBasicSkinAnimationTenView:ctor( ... )
...
init ui
--]]
function CapsuleBasicSkinAnimationTenView:InitUI()
    ...
    local function CreateView()
            ...
            view:runAction(
                cc.Sequence:create(
                    ...
                    cc.CallFunc:create(function()
                        local coinNode = require('Game.views.drawCards.CapsuleBasicSkinAnimationCardNode').new({reward = reward[i]})
        ...
        -- 确定按钮
        local confirmBtn = display.newButton(display.width / 2, 70, {n = RES_DICT.COMMON_BTN})
...
跳过按钮点击回调
--]]
function CapsuleBasicSkinAnimationTenView:SkipButtonCallback( sender )
...
确定按钮点击回调
--]]
function CapsuleBasicSkinAnimationTenView:ConfirmButtonCallback( sender )
    PlayAudioByClickNormal()
    if self.cb then
        self.cb()
    end
...
硬币点击事件处理
--]]
function CapsuleBasicSkinAnimationTenView:CoinClickEvent( stage, signal )
...
更新按钮状态
--]]
function CapsuleBasicSkinAnimationTenView:UpdateButtonState()
    local viewData = self.viewData 
    local coinList = viewData.coinList
    local num = 0
    for i, v in ipairs(coinList) do
...
function CapsuleBasicSkinAnimationTenView:onCleanup()
	--清理逻辑
    AppFacade.GetInstance():UnRegistObserver(CAPSULE_SKIN_COIN_CLICK, self)
end
return CapsuleBasicSkinAnimationTenView