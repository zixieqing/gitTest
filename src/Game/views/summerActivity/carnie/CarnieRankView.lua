...
游乐园（夏活）排行榜view
--]]
local CarnieRankView = class('CarnieRankView', function ()
    local node = CLayout:create(display.size)
    node.name = 'summerActivity.carnie.CarnieRankView'
...
function CarnieRankView:ctor(...)
    ...
    local function CreateView()
        ...
        display.commonLabelParams(tabNameLabel, {reqW = 250, ttf = true, font = TTF_GAME_FONT, text = summerActMgr:getThemeTextByText(__('乐园排行榜')), fontSize = 30, color = '473227',offset = cc.p(0,-8)})
		...
		local endLabel = display.newLabel(28, 600, fontWithColor(16, {text = summerActMgr:getThemeTextByText(__('活动结束剩余时间：')), ap = cc.p(0, 0.5)}))
		...
		timeNum:setPosition(endLabel:getPositionX() + display.getLabelContentSize(endLabel).width + 5, 605)
		timeNum:setAnchorPoint(cc.p(0, 0.5))
        timeNum:setScale(1.2)
        rankLayout:addChild(timeNum, 10)
		local timeLabel = display.newLabel(195+timeNum:getContentSize().width*1.2, 600, fontWithColor(16, {ap = cc.p(0, 0.5), text = summerActMgr:getThemeTextByText(__('天'))}))
		rankLayout:addChild(timeLabel, 10)
		local tipsLabel = display.newLabel(28, 565, fontWithColor(6, {text = summerActMgr:getThemeTextByText(__('排行榜每小时更新一次排名')), ap = cc.p(0, 0.5)}))
		...
		local  playerName = display.newLabel(200, 35, {ap = cc.p(0, 0.5), text = gameMgr:GetUserInfo().playerName, ap = display.LEFT_CENTER,  fontSize = 22, color = '#a87543'})
        ...
        -- 未入榜
		local playerRankLabel = display.newLabel(88, 35, {text = summerActMgr:getThemeTextByText(__('未入榜')), fontSize = 22, color = '#ba5c5c'})
		...
		display.commonLabelParams(rewardBtn, {fontSize = 20, color = 'ffffff', font = TTF_GAME_FONT, ttf = true, outline = '734441', text = summerActMgr:getThemeTextByText(__('查看奖励'))})
...
function CarnieRankView:onCleanup()
end

return CarnieRankView