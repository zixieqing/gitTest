...
function SelectPlayerSkillPopup:InitialUI()
	...
	local function CreateView()
		...
		-- title
		local titleBg = display.newButton(0, 0, {n = _res('ui/common/common_bg_title_2.png'),scale9 = true , animation = false})
		...
		local unlockPlayerSkill = CommonUtils.UnLockModule(RemindTag.TALENT)
		local hintStr = ''
		...
		if needShowHint then
			...
			display.commonLabelParams(dialogue_tips,{text = hintStr, fontSize = 24, color = '#4c4c4c' , w= 360 , hAlign = display.TAC})
	        emptyView:addChild(dialogue_tips, 6)
	        -- 中间小人
		    local loadingCardQ = AssetsUtils.GetCartoonNode(3, dialogue_tips:getContentSize().width + 180, bgSize.height * 0.5)
		...
		-- 操作按钮
		local equipBtn = display.newButton(0, 0, {n = _res('ui/common/common_btn_orange.png'),scale9 = true })
...
function SelectPlayerSkillPopup:InitPlayerSkillData()
	self.equipedPlayerSkills = checktable(self.args.equipedPlayerSkills)
	self.playerSkillData = checktable(self.args.allSkills)
...
function SelectPlayerSkillPopup:RefreshSelectedSkill(index)
	...
	display.commonLabelParams(self.viewData.equipLabel, {text = equipInfo.name ,reqW = 110 })
