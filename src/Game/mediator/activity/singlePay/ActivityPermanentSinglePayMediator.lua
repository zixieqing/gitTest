...
local NAME = "ActivityPermanentSinglePayMediator"
local ActivityPermanentSinglePayMediator = class(NAME, Mediator)
...
--[[
@params table{
	activityData int         活动数据
}
--]]
function ActivityPermanentSinglePayMediator:ctor(params, viewComponent )
	self.super:ctor(NAME,viewComponent)
    local data = params or {}
	self.activityData = checktable(data.activityData) -- 活动数据
	self.activityId = checkint(data.activityId) -- 活动Id
	self.timeStr = data.timeStr -- 剩余时间
	self.isShowStoreCurrency = isElexSdk() -- 是否根据商店刷新货币信息
	...
	elseif isKoreanSdk() then
		self.defaultArea = 'KRW'
		self.defaultCurrencySymbol = '￦'
	end
end

function ActivityPermanentSinglePayMediator:InterestSignals()
	local signals = {
...
function ActivityPermanentSinglePayMediator:ProcessSignal(signal)
...
function ActivityPermanentSinglePayMediator:Initial(key)
	self.super.Initial(self, key)
	local viewComponent  = require('Game.views.activity.singlePay.ActivityPermanentSinglePayView').new({})
	display.commonUIParams(viewComponent, {ap = cc.p(0.5, 0.5), po = cc.p(display.width * 0.5, display.height * 0.5)})
	self:SetViewComponent(viewComponent)
	
    viewComponent.viewData_.gridView:setDataSourceAdapterScriptHandler(handler(self, self.PermanentSinglePayDataSource))
	viewComponent.viewData_.timeLabel:setString(self.timeStr)
...
常驻单笔充值列表处理
--]]
function ActivityPermanentSinglePayMediator:PermanentSinglePayDataSource(p_convertview, idx)
    local pCell = p_convertview
    local index = idx + 1
    if pCell == nil then
        local gridViewCellSize = self:GetViewComponent().viewData_.gridViewCellSize
...
常驻单笔充值活动领取按钮回调
--]]
function ActivityPermanentSinglePayMediator:PermanentDrawButtonCallback(sender)
...
初始化常驻单笔充值活动数据f
--]]
function ActivityPermanentSinglePayMediator:InitPermanentActivityData( data )
 ...
 function ActivityPermanentSinglePayMediator:UpdatePropExchangeList()
    local gridView = self:GetViewComponent().viewData_.gridView
...
领取按钮点击回调
--]]
function ActivityPermanentSinglePayMediator:DrawButtonCallback(sender)
	PlayAudioByClickNormal()
...
function ActivityPermanentSinglePayMediator:UpdatePropList(index)
	local gridView = self:GetViewComponent().viewData_.gridView
...
更新活动页面小红点
--]]
function ActivityPermanentSinglePayMediator:RefreshRemindIcon()
