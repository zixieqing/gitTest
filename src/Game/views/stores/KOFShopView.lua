...
工会商店View
--]]
local KOFShopView = class('KOFShopView', function ()
	local node = CLayout:create(display.size)
	node.name = 'Game.views.stores.KOFShopView'
...
local abg = display.newImageView(_res('ui/home/union/guild_shop_bg_white.png'), 0, 0)
	...
	display.commonLabelParams(titleBg, fontWithColor(18, {text = __('通宝商店')}))
...
local refreshTimeLabel  = display.newLabel(refreshTimeLabelX, refreshTimeLabelY, {text = '00:00:00', fontSize = fontWithColor('16').fontSize, color = fontWithColor('16').color, ap = display.LEFT_CENTER})
    ...
    local moneyNods = {}
    local iconData = {KOF_CURRENCY_ID, HP_ID, GOLD_ID, DIAMOND_ID}
...
function KOFShopView:InitShowUiAndTopUi(layoutData)
	-- dump(showTopUiType)
