...
function BaseSkill:Cast(params)
	...
	if nil == bulletEffectData or self:IsSkillHalo() then
...
function BaseSkill:BaseCast(targetsWithBuffinfo)
	...
	for i, v in ipairs(targetsWithBuffinfo) do
		...
		if true == v.needRevive then
			target = BMediator:GetDeadObjByTag(tTag)
		else
			target = BMediator:IsObjAliveByTag(tTag)
		end
		if nil ~= target then
...
function BaseSkill:ConvertConfigValue2RealValue(caster, target, buffValueConfig, buffInfo, skillExtra, percent)
	if ConfigBuffType.ISD == buffInfo.btype or ConfigBuffType.DOT == buffInfo.btype then
		...
		-- 配置中存在两个值
		if caster and caster.getMainProperty then
	...
	elseif ConfigBuffType.DISPEL_QTE == buffInfo.btype or
		...
		ConfigBuffType.IMMUNE_BUFF_TYPE == buffInfo.btype then
...
function BaseSkill:AddBuffs2Target(target, buffs, actionPhaseCounter, actionPercent, effectPhaseCounter, effectPercent, showHurtEffect)
	...
	local qteBuffsInfo = {
		...
		ownerTag = tTag,
		casterTag = self:GetSkillCasterTag(),
		maxTouch = 0,
		qteBuffs = {},
	} 
	---------- tmp data ----------
	...
	for i, buffInfo_ in ipairs(buffs) do
		...
		if 1 == actionPhaseCounter * effectPhaseCounter then
			...
			if ifCausedEffect and 0 < buffInfo.qteTapTime then
				-- 插入qte信息
				table.insert(qteBuffsInfo.qteBuffs, 1, {qteTapTime = buffInfo.qteTapTime, btype = btype})

				if qteBuffsInfo.maxTouch < buffInfo.qteTapTime then
					qteBuffsInfo.maxTouch = buffInfo.qteTapTime
				end
			end
			---------- 处理qte信息 ----------
	...
	if 1 == actionPhaseCounter * effectPhaseCounter then
		...
		if 0 < causeEffectCounter and 0 < self:GetSkillInfectTime() then
			...
			target:addInfectDriver(infectInfo)
		end
		---------- 传染逻辑 ----------
	end
	---------- 处理传染信息 ----------
		...
		if nil ~= target then
			local bulletData = ObjectSendBulletData.New(
				...
				function (effectPercent, effectPhaseCounter)
					local target_ = nil
					if true == v.needRevive then
						target_ = BMediator:GetDeadObjByTag(v.tag)
					else
						target_ = BMediator:IsObjAliveByTag(v.tag)
					end
...
function BaseSkill:SendAreaEffectBullet(targetsWithBuffinfo, actionPhaseCounter, params)
	------------ 计算目标点 ------------
	local maxp = cc.p(0, 0)
	local minp = cc.p(BMediator:GetBattleRoot():getContentSize().width, BMediator:GetBattleRoot():getContentSize().height)
	local tag = nil
	local target = nil
	local targetP = nil
	...
	for i,v in ipairs(targetsWithBuffinfo) do
		tag = v.tag
		if true == v.needRevive then
			target = BMediator:GetDeadObjByTag(tag)
		else
			target = BMediator:IsObjAliveByTag(tag)
		end
...
function BaseSkill:GetSkillCaster()
	return BMediator:IsObjAliveByTag(self:GetSkillCasterTag())
end
--[[
获取本技能的天气id
--]]
...
function BaseSkill:CanCastBuffBySuccessRate(buffType)
	...
	if QuestBattleType.RAID == BMediator:GetBData():getBattleConstructData().questBattleType then
		return BMediator:GetRandomManager():GetRandomInt(1000) <= buffSuccessRate * 1000
	else
		return math.random(1000) <= buffSuccessRate * 1000
	end
end
--[[
获取buff释放成功率
