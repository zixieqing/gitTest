...
SkillSpineEffectStruct = {
	...
	@params conf table 配表结构 
	--]]
	New = function (skillId, conf)
		...
		this.effectLocation = cc.p(0.5, 0) 					-- 该技能发射特效的坐标修正
		------------ 初始化数据结构 ----------]]--

		this:Init(skillId, conf)
		return this
	end,
	Init = function (self, skillId, conf)
		------------ 转换数据结构 ------------

		self.skillId = skillId
		-- 如果传入的非法值 则初始化成默认状态
		if nil == conf or nil == conf[tostring(skillId)] then
			-- 初始化默认数据结构
			self.actionName = sp.AnimationName.attack
			self.hurtEffectData = {}
			self.attachEffectData = {}
			if -1 ~= skillId then
				self.actionName = sp.AnimationName.skill .. '1'
				local skillConf = CommonUtils.GetSkillConf(skillId)
				assert(skillConf, '\n**************\n cannot find skill conf, skillId : ' .. skillId)
				if skillConf.type and next(skillConf.type) then
					for k,v in pairs(skillConf.type) do
						self.hurtEffectData[k] = {}
						self.attachEffectData[k] = {}
					end
				end
			end
			return
		...
		------------ 动作名 动作id为-1时代表攻击动作 ------------
		local effectConf = conf[tostring(skillId)]
		self.actionName = -1 == skillId and sp.AnimationName.attack or sp.AnimationName.skill .. effectConf.actionId
		------------ 动作名 动作id为-1时代表攻击动作 ------------

		------------ 动作时音效 ------------
		local actionSE = BattleUtils.GetFilteredStringBySpace(effectConf.startEffect)
		if nil ~= actionSE then
			self.actionSE = tostring(effectConf.startEffect)
		else
			self.actionSE = nil
		end

		local actionVoice = BattleUtils.GetFilteredStringBySpace(effectConf.soundCard)
		if nil ~= actionVoice then
			self.actionVoice = tostring(effectConf.soundCard)
		else
			self.actionVoice = nil
		end

		local actionCauseSE = BattleUtils.GetFilteredStringBySpace(effectConf.soundEffect)
		if nil ~= actionCauseSE then
			self.actionCauseSE = tostring(effectConf.soundEffect)
		else
			self.actionCauseSE = nil
		end
		------------ 动作时音效 ------------

		------------ 发射的子弹特效信息 ------------ 
		self.bulletType = ConfigEffectBulletType.BASE
		self.causeType = ConfigEffectCauseType.BASE
		self.effectId = nil
		self.effectActionName = (-1 == checkint(effectConf.effectNo)) and sp.AnimationName.attack or sp.AnimationName.skill .. effectConf.effectNo
		self.effectZOrder = checkint(effectConf.effectLayer)
		self.effectScale = checknumber(effectConf.effectScale)
		self.effectPos = cc.p(0.5, 0)
		if #effectConf.effectLocation == 2 then
			self.effectPos = cc.p(checknumber(effectConf.effectLocation[1]), checknumber(effectConf.effectLocation[2]))
		end
		-- 判断本地文件 是否存在对应的文件 存在则初始化 不存在则保持默认
		-- !!! 注意 !!! ---
		-- 激光类型的命名不同 做出特殊处理
		local fixedActionName = self.effectActionName
		if ConfigEffectBulletType.SPINE_LASER == checkint(effectConf.effectType) then
			fixedActionName = fixedActionName .. sp.LaserAnimationName.laserBody
		end
		-- !!! 注意 !!! ---
		local effectAnimationsData = SpineCache(SpineCacheName.BATTLE):getAnimationsDataByName('effect_' .. effectConf.effectId)
		if nil ~= effectAnimationsData and nil ~= effectAnimationsData[fixedActionName] then
			self.effectId = checkint(effectConf.effectId)
			self.bulletType = checkint(effectConf.effectType)
			self.causeType = checkint(effectConf.causeType)
		else
			self.effectActionName = nil
		end
		------------ 发射的子弹特效信息 ------------
		...
		if -1 == skillId then
			-- 攻击动作
			self.hurtEffectData = {hurtEffectId = nil, hurtEffectPos = nil, hurtEffectZOrder = nil, hurtSEId = nil}
			self.attachEffectData = {attachEffectId = nil, attachEffectPos = nil, attachEffectZOrder = nil, attachSEId = nil}

			local econf = effectConf.effect
			if nil ~= econf then

				------------ 被击爆点 ------------
				if checkint(econf.hitEffect) ~= 0 and
					nil ~= SpineCache(SpineCacheName.BATTLE):getAnimationsDataByName('hurt_' .. checkint(econf.hitEffect)) then
					-- 特效信息
					self.hurtEffectData.hurtEffectId = checkint(econf.hitEffect)
					self.hurtEffectData.hurtEffectZOrder = checkint(econf.hitEffectLayer)
					-- 特效位置
					if table.nums(econf.hitEffectLocation) == 2 then
						self.hurtEffectData.hurtEffectPos = cc.p(tonumber(econf.hitEffectLocation[1]), tonumber(econf.hitEffectLocation[2]))
					else
						self.hurtEffectData.hurtEffectPos = cc.p(0.5, 0)
					end
					-- 爆点音效
					local hurtSEId = BattleUtils.GetFilteredStringBySpace(effectConf.soundHitEffect)
					if nil ~= hurtSEId then
						self.hurtEffectData.hurtSEId = tostring(effectConf.soundHitEffect)
					else
						self.hurtEffectData.hurtSEId = nil
					end
				end
				------------ 被击爆点 ------------

				------------ 附加特效 ------------
				if checkint(econf.addEffect) ~= 0 and
					nil ~= SpineCache(SpineCacheName.BATTLE):getAnimationsDataByName('hurt_' .. checkint(econf.addEffect)) then
					-- 附加效果特效信息
					self.attachEffectData.attachEffectId = checkint(econf.addEffect)
					self.attachEffectData.attachEffectZOrder = checkint(econf.addEffectLayer)
					-- 附加效果特效位置
					if table.nums(econf.addEffectLocation) == 2 then
						self.attachEffectData.attachEffectPos = cc.p(tonumber(econf.addEffectLocation[1]), tonumber(econf.addEffectLocation[2]))
					else
						self.attachEffectData.attachEffectPos = cc.p(0.5, 0)
					end
					-- 附加效果音效
					local attachSEId = BattleUtils.GetFilteredStringBySpace(effectConf.soundAddEffect)
					if nil ~= attachSEId then
						self.attachEffectData.attachSEId = tostring(effectConf.soundAddEffect)
					else
						self.attachEffectData.attachSEId = nil
					end
				end
				------------ 附加特效 ------------
			...
			if skillConf.type and next(skillConf) then
				for k,v in pairs(skillConf.type) do

					self.hurtEffectData[k] = {}
					self.attachEffectData[k] = {}

					local econf = effectConf.effect[k]
					...
					if nil ~= econf then
						...
						if checkint(econf.hitEffect) ~= 0 and
							nil ~= SpineCache(SpineCacheName.BATTLE):getAnimationsDataByName('hurt_' .. checkint(econf.hitEffect)) then
							-- 特效信息
							self.hurtEffectData[k].hurtEffectId = checkint(econf.hitEffect)
							self.hurtEffectData[k].hurtEffectZOrder = checkint(econf.hitEffectLayer)
							-- 特效位置
							if table.nums(econf.hitEffectLocation) == 2 then
								self.hurtEffectData[k].hurtEffectPos = cc.p(tonumber(econf.hitEffectLocation[1]), tonumber(econf.hitEffectLocation[2]))
							else
								self.hurtEffectData[k].hurtEffectPos = cc.p(0.5, 0)
							end
							-- 爆点音效
							local hurtSEId = BattleUtils.GetFilteredStringBySpace(effectConf.soundHitEffect)
							if nil ~= hurtSEId then
								self.hurtEffectData[k].hurtSEId = tostring(effectConf.soundHitEffect)
							else
								self.hurtEffectData[k].hurtSEId = nil
							end
						end
						------------ 被击爆点 ------------
						...
						if checkint(econf.addEffect) ~= 0 and
							nil ~= SpineCache(SpineCacheName.BATTLE):getAnimationsDataByName('hurt_' .. checkint(econf.addEffect)) then
							-- 附加效果特效信息
							self.attachEffectData[k].attachEffectId = checkint(econf.addEffect)
							self.attachEffectData[k].attachEffectZOrder = checkint(econf.addEffectLayer)
							-- 附加效果特效位置
							if table.nums(econf.addEffectLocation) == 2 then
								self.attachEffectData[k].attachEffectPos = cc.p(tonumber(econf.addEffectLocation[1]), tonumber(econf.addEffectLocation[2]))
							else
								self.attachEffectData[k].attachEffectPos = cc.p(0.5, 0)
							end
							-- 附加效果音效
							local attachSEId = BattleUtils.GetFilteredStringBySpace(effectConf.soundAddEffect)
							if nil ~= attachSEId then
	...
	Init = function (self, phaseChangeConf)
		...
		elseif ConfigPhaseType.PLOT == self.phaseType then
			...
			for i, actionConf in ipairs(phaseChangeConf.action) do
				...
				------------ 剧情信息 ------------
				-- pdata.forcePauseGame = checkint(actionConf[1])
...
BattleConstructorStruct = {
	...
	@params enableConnect bool 连携技可用
	@params autoConnect bool 自动释放连携技
	------------ 其他信息 ------------
	...
	New = function (
			------------ 战斗基本配置 ------------
			stageId, questBattleType, randomConfig, gameTimeScale, time, totalWave, resultType, stageCompleteInfo,
			------------ 战斗数值配置 ------------
			levelRolling,
			------------ 战斗环境配置 ------------
			weather, phaseChangeDatas, abilityRelationInfo, globalEffects, enableConnect, autoConnect,
			------------ 其他信息 ------------
...
ObjPFixedAttrStruct = {
	...
	Init = function (self,
		...
		self.pvalue = {
			[ObjP.HP] 				= checknumber(hpValue or -1),
		}
