...
function BattleFailView:InitCommonLayer()
	...
	display.commonUIParams(hintBg, {po = cc.p(
		layerSize.width - hintBg:getContentSize().width * 0.5 - 10,
		layerSize.height * 0.5 - 175
	)})
	commonLayer:addChild(hintBg, 2)
	hintBg:setCascadeOpacityEnabled(true)

	local hintBgSize = hintBg:getContentSize()

	local hintLabel = display.newLabel(
		hintBgSize.width * 0.5, hintBgSize.height - 2,
		fontWithColor('9', {ap = cc.p(0.5, 1), text = __('通过以下方式提升自己')})
	)
	hintBg:addChild(hintLabel)
	local hintLabelSize = display.getLabelContentSize(hintLabel)
	if hintLabelSize.width > 450  then
		display.commonLabelParams(hintLabel , {w = 500, reqW = 420, hAlign = display.TAC})
	end
	...
	for i,v in ipairs(strongInfo) do
		...
		local avatarLabel = display.newLabel(0, 0, fontWithColor('19', {text = v.descr, reqW = 180}))
