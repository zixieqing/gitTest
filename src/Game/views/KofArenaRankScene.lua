...
---@class KofArenaRankScene
local KofArenaRankScene = class('KofArenaRankScene', GameScene)
...
function KofArenaRankScene:ctor(...)
    self.super.ctor(self,'views.KofArenaRankScene')
    self.viewData = nil
    local function CreateView()
        ...
        display.commonLabelParams(tabNameLabel, {ttf = true, font = TTF_GAME_FONT, text = __('天城演武排行榜'), reqW = 240 ,  fontSize = 30, color = '473227',offset = cc.p(0,-8)})
        ...
        -- 上周奖励排行按钮
        local lastWeekRankBtn = display.newButton(titleBgSize.width - 350  ,titleBgSize.height/2 , { scale9 = true ,  tag = BUTTON_TAG.LAST_WEEK_RANK_BTN, n = _res('ui/common/common_btn_white_default.png')})
        topLayout:addChild(lastWeekRankBtn)
        display.commonLabelParams(lastWeekRankBtn, fontWithColor('14' ,{text =__('上周排行榜'), fontSize = 22,paddingW = 20  }))

        -- 查看奖励的btn
        local rewardBtn = display.newButton(titleBgSize.width - 150 , titleBgSize.height/2, { scale9 = true , tag = BUTTON_TAG.LOOK_REWARDS, n = _res('ui/common/common_btn_orange.png')})
        topLayout:addChild(rewardBtn )
        display.commonLabelParams(rewardBtn, fontWithColor('14' ,{text =__('查看奖励') , fontSize = 22 ,  paddingW = 20 }))
...
function KofArenaRankScene:onCleanup()

end

return KofArenaRankScene