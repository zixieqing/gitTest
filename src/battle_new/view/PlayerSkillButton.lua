...
初始化点击回调
--]]
function PlayerSkillButton:InitClickHandler()
	display.commonUIParams(self, {cb = handler(self, self.PlayerSkillButtonClickHandler)})
end
--[[
技能按钮点击回调
--]]
function PlayerSkillButton:PlayerSkillButtonClickHandler(sender)
	PlayAudioByClickNormal()
	if nil ~= self.args.callback then
		local skillId = self:GetSkillId()
		local logicTag = self:GetLogicTag()
		self.args.callback(logicTag, skillId)
	end
end
---------------------------------------------------
-- click handler end --
...
获取技能id
--]]
function PlayerSkillButton:GetSkillId()
	return self.args.skillId
end
--[[
获取逻辑层tag
--]]
function PlayerSkillButton:GetLogicTag()
	return self.args.logicTag
end
---------------------------------------------------
-- get set end --
---------------------------------------------------
