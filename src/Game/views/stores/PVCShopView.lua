...
勋章View
--]]
local PVCShopView = class('PVCShopView', function ()
	local node = CLayout:create(display.size)
	node.name = 'Game.views.stores.PVCShopView'
...
local function CreateView( )
	...
	display.commonLabelParams(titleBg, fontWithColor(18, {text = __('勋章商店')}))
    ...
    local moneyNods = {}
    local iconData = {PVC_MEDAL_ID, HP_ID, GOLD_ID, DIAMOND_ID}
    for i,v in ipairs(iconData) do
		...
		if v == PVC_MEDAL_ID then isDisable = true end
...
function PVCShopView:InitShowUiAndTopUi(layoutData)
	-- dump(showTopUiType)
