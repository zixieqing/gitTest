...
local CardContractCompleteMediator = class("CardContractCompleteMediator", Mediator)

local NAME = "CardContractCompleteMediator"
...
function CardContractCompleteMediator:ctor(param, viewComponent )
...
function CardContractCompleteMediator:InterestSignals()
	local signals = {
	}
...
function CardContractCompleteMediator:ProcessSignal(signal )
	local name = signal:GetName()
	-- print(name)
	-- dump(signal:GetBody())
end

function CardContractCompleteMediator:Initial( key )
	self.super.Initial(self,key)
	local scene = uiMgr:GetCurrentScene()
	local viewComponent  = require( 'Game.views.CardContractCompleteView' ).new(self.cardData)
	...
	viewData.memoryButton:setOnClickScriptHandler(function(sender)
		PlayAudioByClickNormal()
		
		local mediator = require( 'Game.mediator.CardMemoryMediator' ).new({data = self.cardData, cb = NAME})
	...
	local function startViewAction()
		...
		local unlockStr = {__('飨灵故事' .. 'Ⅴ'), __('触摸语音'), __('专属昵称')}
		if CardUtils.IsLinkCard(self.cardData.cardId) then
			unlockStr = {}
		end
		viewComponent:AddBuffDesr(str, unlockStr)
		PlayAudioClip(AUDIOS.UI.ui_vow_settlement.id)
	end
	startViewAction()
end

function CardContractCompleteMediator:GetBuffDes( favorabilityLevel )
...
function CardContractCompleteMediator:OnRegist(  )
end

function CardContractCompleteMediator:OnUnRegist(  )
	local scene = uiMgr:GetCurrentScene()
	scene:RemoveGameLayer(self.viewComponent)
end

return CardContractCompleteMediator
