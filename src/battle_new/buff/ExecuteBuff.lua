...
function ExecuteBuff:CauseEffect()
	...
	if nil ~= owner then
		-- 伤害免疫时免疫斩杀
		if owner:DamageImmuneByDamageType(DamageType.SKILL_PHYSICAL) or
			owner:IsObjectImmuneBuff(ConfigBuffType.EXECUTE) then
			return 0
		end
		...
		-- 如果免疫斩杀 则只产生基础的伤害
		if self.p.percent * 10000 >= math.ceil(targetHpPercent * 10000) and not owner:IsObjectImmuneBuff(btype) then

			-- 满足斩杀条件
			damageData:SetDamageValue(math.max(owner:GetMainProperty():GetCurrentHp(), owner:GetMainProperty():GetOriginalHp()))
