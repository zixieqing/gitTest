...
local Anniversary20ReviewAnimationView = class('Anniversary20ReviewAnimationView', function()
    return display.newLayer(0, 0, {name = 'Game.views.anniversary20.Anniversary20ReviewAnimationView'})
end)

local RES_DICT = {
    SKIP_BTN = _res('arts/stage/ui/opera_btn_skip.png'),
    BG_IMAGE = _res('ui/cards/marry/card_contract_bg_memory.jpg'),
}
...
function Anniversary20ReviewAnimationView:ctor(args)
...
CreateView = function() 
    ...
    display.commonLabelParams(skipBtn, fontWithColor(20, {text = __('跳过'), fontSize = 24, ap = display.RIGHT_CENTER, offset = skipOffP}))
...
CreatePage = function(pageName)
    ...
    local pageFile = _res(string.format('ui/anniversary20/reviewAnimate/%s.csb', tostring(pageName)))
...
-------------------------------------------------
-- get / set
function Anniversary20ReviewAnimationView:getViewData()
...
function Anniversary20ReviewAnimationView:close()
    -- clean page
    self:cleanPage()
...
function Anniversary20ReviewAnimationView:cleanPage()
    -- stop listen
...
function Anniversary20ReviewAnimationView:playPage(pageName)
    -- clean page
    self:cleanPage()
    ...
    if self.pageViewData_.timeline then
        local onFrameEvent = function(frame)
            ...
            if string.sub(frameName, 0, BGM_NAME_LENGTH) == BGM_NAME_PREFIX then
                ...
                local audioPath = _res(string.fmt('ui/anniversary20/reviewAnimate/mp3/%1.mp3', frameName))
        ...
        self:eachPageNode_(self.pageViewData_.pageView, function(node)
            ...
            elseif string.sub(nodeName, 0, SPINE_NAME_LENGTH) == SPINE_NAME_PREFIX then
                local spinePath = _spn(string.fmt('ui/anniversary20/reviewAnimate/spine/%1', nodeName))
...
function Anniversary20ReviewAnimationView:playAudio_(audioPath)
...
function Anniversary20ReviewAnimationView:stopAudio_()
    cc.SimpleAudioEngine:getInstance():stopMusic(true)
end


function Anniversary20ReviewAnimationView:hideSkip_(isFast)
...
function Anniversary20ReviewAnimationView:showSkip_(isFast, isForever)
    self.isControllable_ = false
    local finishCB = function()
        self.isControllable_ = true
...
function Anniversary20ReviewAnimationView:eachPageNode_(pageView, eachFunc)
...
function Anniversary20ReviewAnimationView:startCheckPlayEnd_()
...
function Anniversary20ReviewAnimationView:stopCheckPlayEnd_()
...
function Anniversary20ReviewAnimationView:onClickSkipHotspotHandler_(sender)
...
function Anniversary20ReviewAnimationView:onClickCloseButtonHandler_(sender)
...
return Anniversary20ReviewAnimationView
