...
local ChooseBattleHeroView = class('ChooseBattleHeroView', GameScene)
...
function ChooseBattleHeroView:ctor( ... )
	local arg = unpack({...})
    self.datas = arg
	...
	local function CreateView( ... )
	    ...
	    --标题  物品名称
	    local btn_up = display.newButton(0, 0, {n = _res(RES_DICT.BG_UP),enable = false})
		display.commonUIParams(btn_up, {po = cc.p(pox, frameSize.height - 12),ap = cc.p(0.5,1)})
		display.commonLabelParams(btn_up, fontWithColor(1,{fontSize = 24, text = __('选择飨灵'), color = 'ffffff',offset = cc.p(0, -2)}))
		...
		local nameAndFigthLabel = display.newLabel(utils.getLocalCenter(nameAndFigthBtn).x, utils.getLocalCenter(nameAndFigthBtn).y,
				fontWithColor(5,{text = __('显示灵力'), color = 'ffffff' ,reqW = 120 , w = 180 , hAlign = display.TAC }))
		nameAndFigthBtn:addChild(nameAndFigthLabel)
		if isJapanSdk() then
			display.commonLabelParams(nameAndFigthLabel,fontWithColor(5,{text = __('显示灵力'), color = 'ffffff' , hAlign = display.TAC }))
		end


		local captainDesLabel = display.newLabel(220,555,fontWithColor('15', {ap = display.LEFT_CENTER , hAlign = display.TAL, text = __('队长奖励：') .. __("进入战斗时获得50能量") , w = 450 }))
		...
		for i,v in ipairs(screenType) do
			...
			if v.tag ~= 0 then
				...
				local careerBg = display.newNSprite(_res(CardUtils.CAREER_ICON_FRAME_PATH_MAP[tostring(v.tag)]), centerPos.x - 25, centerPos.y)
...
显示切换卡牌灵力和名字
--]]
function ChooseBattleHeroView:NameAndFigthBtnCallback(sender)
...
筛选按钮回调
--]]
function ChooseBattleHeroView:ScreenBtnCallback(sender)
    PlayAudioByClickNormal()
	local checked = sender:isChecked()
...
--]]
function ChooseBattleHeroView:ShowScreenBoard(visible)
...
筛选按钮点击回调
--]]
function ChooseBattleHeroView:ScreenTypeBtnCallback(sender)
    PlayAudioByClickNormal()
	local tag = sender:getTag()
	self:ScreenCards(tag)
	display.commonLabelParams(self.viewData_.screenLabel , {reqW =120 , text = screenType[tag + 1].descr or CardUtils.GetCardCareerName(tag)})
...
4 所有治疗
--]]
function ChooseBattleHeroView:ScreenCards(pattern)
	local tag = pattern
	self.Tdata = {}
...
排序按钮回调
--]]
function ChooseBattleHeroView:SortBtnCallback(sender)
    PlayAudioByClickNormal()
	local checked = sender:isChecked()
...
--]]
function ChooseBattleHeroView:ShowSortBoard(visible)
...
排序按钮点击回调
--]]
function ChooseBattleHeroView:SortTypeBtnCallback(sender)
    PlayAudioByClickNormal()
	local tag = sender:getTag()
	self.sortBtnState[tag+1] = sender:isChecked()
	display.commonLabelParams(self.viewData_.sortLabel ,{text = sortType[tag + 1].descr , reqH = 40 , w = 140 , hAlign= display.TAC})
...
function ChooseBattleHeroView:SortAction(tag,btn,arrowImg)
	if tag ~= 1 then
...
function ChooseBattleHeroView:SortTableByRule(cardsData  , sortRule , isUpAndDown)
	local r = nil
...
function ChooseBattleHeroView:OnDataSourceAction(p_convertview,idx)
    ...
    if nil ==  pCell  then
		...
		cardHeadNode:RefreshUI({specialType = self.Tdata[index].specialType ,
			...
			showActionState = true,showName = true,showNameOrFight = self.showNameOrFight})
		cardHeadNode:setOnClickScriptHandler(handler(self,self.cellCallBackActions))
    end
    xTry(function()
    	cardHeadNode:RefreshUI({specialType = self.Tdata[index].specialType ,
...
--]]
function ChooseBattleHeroView:cellCallBackActions( sender  )
    PlayAudioByClickNormal()
	local tag = sender:getParent():getTag()
	if tag == 1 and  self.datas.id   then
		gameMgr:DeleteTempFormationDataById(self.datas.id)
		...
		local removeActionFunc = function ()
			if self.callback then
				self.datas = {}
		...
		local places = gameMgr:GetCardPlace({id = self.Tdata[tag].id})
		...
		if places and table.nums(places) > 0 then
			...
			elseif places[tostring(CARDPLACE.PLACE_FISH_PLACE)]  then
				uiMgr:ShowInformationTips(__('该飨灵正在钓场中。'))
				return
			elseif places[tostring(CARDPLACE.PLACE_FISH_PLACE)]   then
				uiMgr:ShowInformationTips(__('该飨灵正在好友钓场中。'))
				return
			elseif places[tostring(CARDPLACE.PLACE_ASSISTANT)] then
				if GuideUtils.IsGuiding() and GuideUtils.GetGuidingId() == GUIDE_MODULES.MODULE_TEAM  then
					removeActionFunc()
					self:runAction(cc.RemoveSelf:create())
					GuideUtils.DispatchStepEvent()
				end
				uiMgr:ShowInformationTips(__('该飨灵正在大堂工作，不能上阵。'))
				return
			else
				if places[tostring(CARDPLACE.PLACE_TEAM)] then
					if GuideUtils.IsGuiding() and GuideUtils.GetGuidingId() == GUIDE_MODULES.MODULE_TEAM  then
						...
						local temp_str = __('确定将该飨灵放入编队？')
						local CommonTip  = require( 'common.CommonTip' ).new({descr = __('该飨灵已经在编队中，是否将其移至当前编队。'),text = temp_str,isOnlyOK = false, callback = function ()
							removeActionFunc()
							self:runAction(cc.RemoveSelf:create())
						end})
...
--[[
	刷新界面信息
--]]
function ChooseBattleHeroView:RefreshUI()
	print('ChooseBattleHeroView:RefreshUI()')

end

function ChooseBattleHeroView:onEnter()
    -- add touch listener
end

function ChooseBattleHeroView:onExit()
	local scene = uiMgr:GetCurrentScene()
	scene:RemoveDialogByName('CommonTip')
end


return ChooseBattleHeroView
