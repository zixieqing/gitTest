...
function BossWeakScene:init()
	...
	self.touchWeakPointCB = self.args.touchWeakPointCB
	self.ownerViewModelTag = self.args.ownerViewModelTag
...
function BossWeakScene:initWeakLayer()
	...
	for i, v in ipairs(self.args.weakPoints) do
		...
		local bossWeakSpine = SpineCache(SpineCacheName.BATTLE):createWithName(sp.AniCacheName.BOSS_WEAK_POINT)
...
function BossWeakScene:onTouchBegan_(touch, event)
	if G_BattleRenderMgr and G_BattleRenderMgr:IsBattleTouchEnable() and self:isVisible() and self.gameStart then
...
function BossWeakScene:touchedItemHandler(id)
	...
	if self.gameStart then
		...
		if nil ~= self.touchWeakPointCB then
			self.touchWeakPointCB(self:GetSceneTag(), id)
		end
...
@params id int 弱点id
--]]
function BossWeakScene:WeakPointBomb(id)
	if not id then return end
	...
	self.weekPointBombTime = SpineCache(SpineCacheName.BATTLE):getAnimationsDataByName(sp.AniCacheName.BOSS_WEAK_POINT)[bombEffectActionName].duration
...
function BossWeakScene:updateWeakLocation()
	local weakPoint = nil
	local boneData = nil
	local ownerView = self:GetOwnerRenderView()

	if nil ~= ownerView then

		for k,v in pairs(self.touchItems) do
			local boneWorldPos = ownerView:FindBoneInAvatarByWorldPos(v.bone)

			if nil ~= boneWorldPos then
				display.commonUIParams(v.node, {po = v.node:getParent():convertToNodeSpace(boneWorldPos)})
			end
		end
...
获取本体展示层模型
--]]
function BossWeakScene:GetOwnerViewModelTag()
	return self.ownerViewModelTag
end
--[[
获取owner物体的渲染层模型
--]]
function BossWeakScene:GetOwnerRenderView()
	return G_BattleRenderMgr:GetAObjectView(self:GetOwnerViewModelTag())
end
---------------------------------------------------
-- get set end --
---------------------------------------------------
