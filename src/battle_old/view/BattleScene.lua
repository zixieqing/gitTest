...
function BattleScene:InitUI()
	...
	local function CreateView()
		...
		local battleLayer = display.newLayer(0, 0, {size = cc.size(display.SAFE_RECT.width, display.SAFE_RECT.height)})
		...
		actionButtons[tostring(1001)] = pauseButton
		...
		actionButtons[tostring(1002)] = accelerateButton
	...
	xTry(function ( )
        ...
        audioManager:PlayBGMusic(AUDIOS.BGM.name, AUDIOS.BGM.Food_Battle.id, true)
...
--[[
初始化主角技相关ui
@params energyLabel cc.Node 能量标签
--]]
function BattleScene:InitPlayerView()
	-- 主角技底板
	local playerSkillBg = display.newImageView(_res('ui/battle/battle_bg_lead_skill.png'), 0, 0)

	local playerSkillLayer = display.newLayer(display.SAFE_L - 60, 0, {ap = cc.p(0, 0), size = playerSkillBg:getContentSize()})
	-- playerSkillLayer:setBackgroundColor(cc.c4b(255, 255, 64, 128))
	self.viewData.uiLayer:addChild(playerSkillLayer)

	display.commonUIParams(playerSkillBg, {po = cc.p(0, 0), ap = cc.p(0, 0)})
	playerSkillLayer:addChild(playerSkillBg)

	-- 主角能量
	local playerEnergyBar = CProgressBar:create(_res('ui/battle/battle_bg_skill_energy_line_1.png'))
	playerEnergyBar:setBackgroundImage(_res('ui/battle/battle_bg_skill_energy_line_2.png'))
	...
	local playerEnergyBarLight = display.newNSprite(_res('ui/battle/battle_bg_skill_energy_light.png'), utils.getLocalCenter(playerEnergyBar).x, utils.getLocalCenter(playerEnergyBar).y)
	playerEnergyBar:addChild(playerEnergyBarLight, -1)

	playerEnergyBarLight:setVisible(false)

	local energyLabel = display.newLabel(playerEnergyBar:getPositionX(), playerEnergyBar:getPositionY(),
		fontWithColor(9,{text = '0'}))
	...
	--[[
	{
		{playerSkillIconFrame = nil, playerSkillEnergyFrame = nil, playerSkillButton = nil},
		{playerSkillIconFrame = nil, playerSkillEnergyFrame = nil, playerSkillButton = nil},
		...
	}
	--]]
	self.viewData.playerSkillHintBtns = {}
	-- 初始化技能图标
	for i,v in ipairs(playerSkillIconInfo) do

		local playerSkillIconFrame = display.newNSprite(_res('ui/battle/battle_bg_skill_default.png'), v.skillIconPos.x, v.skillIconPos.y)
		playerSkillBg:addChild(playerSkillIconFrame)

		local playerSkillHintBtn = display.newButton(0, 0, {size = playerSkillIconFrame:getContentSize(), animte = false})
		display.commonUIParams(playerSkillHintBtn, {po = cc.p(v.skillIconPos.x, v.skillIconPos.y)})
		playerSkillLayer:addChild(playerSkillHintBtn, 5)
		playerSkillHintBtn:setVisible(false)
		self.viewData.playerSkillHintBtns[i] = playerSkillHintBtn

		local playerSkillEnergyFrame = display.newNSprite(_res('ui/battle/battle_bg_skill_energy.png'),
			v.skillIconPos.x + v.energyIconPos.x,
			v.skillIconPos.y + v.energyIconPos.y)
		playerSkillLayer:addChild(playerSkillEnergyFrame, 10)

		self.viewData.playerSkillIcons[i] = {
			playerSkillIconFrame = playerSkillIconFrame,
			playerSkillEnergyFrame = playerSkillEnergyFrame,
			playerSkillButton = nil
		}
	end

	self.viewData.playerSkillLayer = playerSkillLayer
	self.viewData.playerSkillBg = playerSkillBg
	self.viewData.playerEnergyBar = playerEnergyBar
	self.viewData.energyLabel = energyLabel
	self.viewData.playerEnergyBarLight = playerEnergyBarLight
end
--[[
添加主角技按钮
...
@return playerSkillButton cc.Node 主角技按钮 
--]]
function BattleScene:AddPlayerSkillButton(skillId, params)
	local playerSkillIconInfo = {
		[1] = {skillIconPos = cc.p(78 + 60, 188), energyIconPos = cc.p(-41, -39)},
		[2] = {skillIconPos = cc.p(185 + 60, 81), energyIconPos = cc.p(-41, -39)}
	}
	...
	-- 创建技能按钮
	local playerSkillButton = __Require('battle.view.PlayerSkillButton').new({skillId = skillId})
	local buttonSize = playerSkillButton:getContentSize()
	local pos = playerSkillIconInfo[params.index].skillIconPos
	display.commonUIParams(playerSkillButton, {
		po = pos,
		cb = params.callback})
	self.viewData.playerSkillLayer:addChild(playerSkillButton, 5)

	self.viewData.playerSkillIcons[params.index].playerSkillButton = playerSkillButton

	-- 创建技能能量消耗
	local playerSkillEnergyFrame = self.viewData.playerSkillIcons[params.index].playerSkillEnergyFrame
	local playerSkillEnergyCostLabel = display.newLabel(utils.getLocalCenter(playerSkillEnergyFrame).x, utils.getLocalCenter(playerSkillEnergyFrame).y,
		{text = params.energyCost, fontSize = 20, color = '#ffffff'})
...
function BattleScene:InitAliveStageClear(countdown)
	if nil == self.viewData.aliveTimeBg then
		...
		display.commonUIParams(aliveLabel, {ap = cc.p(1, 1), po = cc.p(
			aliveTimeBg:getContentSize().width - 5,
...
function BattleScene:ShowBattleFunctionModule(battleFunctionModuleType, show)
	...
	elseif ConfigBattleFunctionModuleType.PLAYER_SKILL == battleFunctionModuleType then
		...
		-- 隐藏主角技模块
		self.viewData.playerSkillLayer:setVisible(show)
	...
	if true then
		...
		local skillIconBtn = display.newButton(0, 0, {size = GlobalEffectIconSize, cb = function (sender)
			...
			if nil ~= skillConfig then
				uiMgr:ShowInformationTipsBoard({targetNode = sender, title = skillConfig.name, descr = skillConfig.descr, type = 5})
			end
		end})
