...
local NewKofArenaFightPrepareView = class('NewKofArenaFightPrepareView', function ()
	local node = CLayout:create(display.size)
	node.name = 'Game.views.tagMatchNew.NewKofArenaFightPrepareView'
...
function NewKofArenaFightPrepareView:ctor( ... )
    self.args = unpack({...})
    self:initData()
    self:initialUI()
end

function NewKofArenaFightPrepareView:initData()
    
end

function NewKofArenaFightPrepareView:initialUI()
    xTry(function ( )
...
function NewKofArenaFightPrepareView:refreshUI(data)
    local playerAttackData = data.playerAttackData
    self:updatePlayerLineup(playerAttackData)
    
    local opponentData      = data.opponentData
    self:updateOpponentLineup(opponentData)

    self:updateMyTeamTypeIcon(checkint(data.attackTeamId) > 0)
end

function NewKofArenaFightPrepareView:updatePlayerLineup(playerAttackData)
...
function NewKofArenaFightPrepareView:updateOpponentLineup(opponentData)
    local viewData       = self:getViewData()
    ...
    local totalBattlePoint = self:updateAllTeamView(teamViews, opponentData.team, true)

    local teamTitleLayer = opponentLineupViewData.teamTitleLayer
    self:updateTeamTitle(teamTitleLayer, totalBattlePoint, opponentData)
end

function NewKofArenaFightPrepareView:updateMyTeamTypeIcon(isPresetTeam)
    local viewData     = self:getViewData()
    local playerLineup = viewData.playerLineup
    local playerLineupViewData = playerLineup.viewData
...
function NewKofArenaFightPrepareView:updateTeamTitle(teamTitleLayer, totalMana, playerData)
    ...
    if rivalHeadNode then
        ...
        local avatar      = playerData.avatar
        local avatarFrame = playerData.avatarFrame
    ...
    if playerNameLabel then
        display.commonLabelParams(playerNameLabel, {text = tostring(playerData.name)})
    end
    
end

function NewKofArenaFightPrepareView:updateAllTeamView(teamViews, teamDatas, isOppoentTeam)
...
function NewKofArenaFightPrepareView:showUIAction(cb)
    local viewData               = self:getViewData()
    local playerLineup           = viewData.playerLineup
    local playerLineupViewData   = playerLineup.viewData
...
function NewKofArenaFightPrepareView:getViewData()
	return self.viewData_
end

return NewKofArenaFightPrepareView