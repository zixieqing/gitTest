...
local NewKofArenaDefensiveLineupView = class('NewKofArenaDefensiveLineupView', function ()
	local node = CLayout:create(display.size)
	node.name = 'Game.views.tagMatchNew.NewKofArenaDefensiveLineupView'
...
function NewKofArenaDefensiveLineupView:ctor( ... )
    self.args = unpack({...}) or {}
    
    self:initialUI()
end

function NewKofArenaDefensiveLineupView:initialUI()
    xTry(function ( )
...
function NewKofArenaDefensiveLineupView:initData()
    self.defendCards = self.args.defenseTeams or {}
    self.isOpponentTeam = self.args.isOpponentTeam
end

function NewKofArenaDefensiveLineupView:initView()
...
function NewKofArenaDefensiveLineupView:updateTeamViews()
    local viewData = self:getViewData()
...
function NewKofArenaDefensiveLineupView:onClickShallowAction(sender)
...
function NewKofArenaDefensiveLineupView:onEnter()
...
function NewKofArenaDefensiveLineupView:CreateTeamView(teamId, teamCards)
    local layer = display.newLayer(0, 0, {bg = RES_DIR.DEFEND_BG})
    local layerSize = layer:getContentSize()
    local teamData = self.isOpponentTeam and {cards = teamCards, battlePoint = teamCards.battlePoint} or teamCards
    local teamView = require("Game.views.tagMatch.TagMatchDefensiveTeamView").new({size = layerSize, teamId = teamId, teamDatas = teamData, teamMarkPosSign = 1, isOppoentTeam = self.isOpponentTeam})
...
function NewKofArenaDefensiveLineupView:getViewData()
	return self.viewData_
end

return NewKofArenaDefensiveLineupView