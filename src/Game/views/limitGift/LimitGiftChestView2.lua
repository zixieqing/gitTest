...
限时超得活动view
--]]
---@class LimitChestGiftView2
local LimitChestGiftView2 = class('LimitChestGiftView2', function ()
    local node = CLayout:create(display.size)
    node:setAnchorPoint(cc.p(0, 0))
    node.name = 'home.LimitChestGiftView2'
...
function LimitChestGiftView2:CreateView()
    ...
    local qAvatar = sp.SkeletonAnimation:create('effects/xsth_effect.json', 'effects/xsth_effect.atlas', 1.0)
    ...
    -- 第一个小人
    local onePeople = sp.SkeletonAnimation:create('ui/home/activity/limitGift/skeleton.json', 'ui/home/activity/limitGift/skeleton.atlas', 1.0)
    onePeople:setPosition(cc.p(bgSize.width/2 , bgSize.height /2 + 5))

    onePeople:setVisible(false)
    ...
    -- 价格label
    local priceLbale = display.newLabel(bgSize.width /2 , 150 , {text =string.format( __('原价￥%s') , "648"  )  , reqW = 220 ,  fontSize = 28 , color = "#5b3c25" } )
...
-- 展的动画播放
function LimitChestGiftView2:ExpandAction()
    ...
    bgParentLayout:runAction(
        cc.Sequence:create(
            cc.CallFunc:create(function ()
                self.isAction = true
                onePeople:setVisible(true )
                onePeople:setToSetupPose()
                onePeople:setAnimation(0, 'play', false)
            end),
...
-- 关闭的动画
function LimitChestGiftView2:CloseAction()
    local viewData_ = self.viewData_
    local bgLayout = viewData_.bgLayout
    local bgParentLayout = viewData_.bgParentLayout
    local mode = 80
...
-- 创建礼包获取的内容
function LimitChestGiftView2:needGoods(data)
    data = data or {}
    -- 删除所有的节点
...
function LimitChestGiftView2:ctor( ... )
    self.isAction = true
    self:CreateView()
