...
local BattleReadyView = class('BattleReadyView', function ()
	local node = CLayout:create(display.size)
	node.name = 'Game.views.BattleReadyView'
    node:setName('BattleReadyView')
...
function BattleReadyView:destory()
	AppFacade.GetInstance():UnRegsitMediator('EnterBattleMediator')
end
--[[
init ui
--]]
function BattleReadyView:InitUI()
	...
	local hintLabel = display.newLabel(0, 0,
		{text = __('点击技能图标更改主角技'), fontSize = 20, color = '#c8b3af', ap = display.LEFT_CENTER , w = 460})
	display.commonUIParams(hintLabel, {po = cc.p(10, display.getLabelContentSize(hintLabel).height * 0.5 + 2)})
	...
	if self.stageId and stageConf then
		costHpLabel = display.newLabel(0, 0,
			fontWithColor(9, {text = string.format(__('消耗%d'), checkint(stageConf.consumeHp))}))
		self:addChild(costHpLabel, 5)

		local costHpIconPath = CommonUtils.GetGoodsIconPathById(HP_ID)
		local isQuest = false
		...
		if checkint(stageConf.consumeHp)  == 0  and (not  isQuest)   then
			costHpIcon:setVisible(false)
			costHpLabel:setVisible(false)
		end
	...
	if 2 == self.battleType then
		...
		self:InitStageInfo()
	elseif 3 == self.battleType  or self.battleType == QuestBattleType.WONDERLAND  then
		self.centerContentData = {
			-- {name = __('队伍'), tag = 1, initHandler = handler(self, self.InitTeamFormationPanel), showHandler = handler(self, self.ShowTeamFormationPanel)},
			-- {name = __('主角技'), tag = 1, initHandler = handler(self, self.InitPlayerSkillPanel), showHandler = handler(self, self.ShowPlayerSkillPanel)}
		}
...
function BattleReadyView:GetDoubleActivitySpine()
	local data = {}
	local questBattleType =  CommonUtils.GetQuestBattleByQuestId(checkint(self.stageId))
	local questConfig = CommonUtils.GetQuestConf(self.stageId)
	local difficulty = checkint(questConfig.difficulty)
	if checkint(QuestBattleType.MAP) ==  checkint(questBattleType) then
		if QUEST_DIFF_HARD == difficulty  then
			if checkint(self.stageId ) >=checkint(gameMgr:GetUserInfo().newestHardQuestId)   then
				return
			end
		elseif QUEST_DIFF_NORMAL ==  difficulty   then
			if checkint(self.stageId ) >= checkint(gameMgr:GetUserInfo().newestQuestId )  then
				return
			end
		end
		if difficulty == QUEST_DIFF_NORMAL then
			data = app.activityMgr:GetActivityDataByType(ACTIVITY_TYPE.DOUNBLE_EXP_NORMAL)
		elseif difficulty == QUEST_DIFF_HISTORY or 	self.selectedDiffType == QUEST_DIFF_HARD  then
...
初始化编队信息
--]]
function BattleReadyView:InitFormationContent()
...
初始化编队信息以外的携带物品信息
--]]
function BattleReadyView:InitBelongings()
	...
	for i,v in ipairs(self.centerContentData) do
		...
		local tabBtn = display.newCheckBox(0, 0, {n = _res('ui/common/maps_fight_btn_tab_default.png'), s = _res('ui/common/maps_fight_btn_tab_select.png') })
		--self:addChild(tabBtn, 20)
		local tabBtnSize = tabBtn:getContentSize()
		local tabBtnLayoutSize = cc.size (tabBtnSize.width * 1.2 , tabBtnSize.height * 1.2)
		local tableLayout  = display.newLayer(
				centerBgPos.x - bgSize.width * 0.5 + tabBtnLayoutSize.width * 0.5 + (i - 1) * (5 + tabBtnLayoutSize.width),
				centerBgPos.y + bgSize.height * 0.5 + tabBtnLayoutSize.height * 0.5 + 10 ,{size = tabBtnLayoutSize , ap = display.CENTER}
		)
		self:addChild(tableLayout , 20 )
		tabBtn:setScaleY(1.5)
		tabBtn:setScaleX(1.2)
		tabBtn:setTag(v.tag)
		table.insert(self.viewData.centerContentTabBtns, tabBtn)

		tabBtn:setOnClickScriptHandler(handler(self, self.ChangeCenterContentBtnCallback))

		tableLayout:addChild(tabBtn)
		display.commonUIParams(tabBtn, { po = cc.p(tabBtnLayoutSize.width/2 , tabBtnLayoutSize.height/2)})

		local tabBtnLabel = display.newLabel(utils.getLocalCenter(tableLayout).x, utils.getLocalCenter(tableLayout).y,
		fontWithColor(12,{text = v.name , reqW =170}))
		tableLayout:addChild(tabBtnLabel)
...
初始化主角技选择
--]]
function BattleReadyView:InitPlayerSkillPanel()
	local bgSize = self.viewData.belongsBgFrame:getContentSize()
	local centerBgPos = cc.p(self.viewData.belongsBgFrame:getPositionX(), self.viewData.belongsBgFrame:getPositionY())
...
--]]
function BattleReadyView:ShowPlayerSkillPanel(visible)
...
换技能按钮回调
--]]
function BattleReadyView:ChangePlayerSkillCallback(sender)
...
刷新主角技装备情况
--]]
function BattleReadyView:RefreshPlayerSkillPanel()
	local skillId = 0
    if gameMgr:GetUserInfo().skill and type(gameMgr:GetUserInfo().skill) == 'table' then
        for i, v in ipairs(gameMgr:GetUserInfo().skill) do
            skillId = checkint(v)
...
初始化编队信息
--]]
function BattleReadyView:InitTeamFormationPanel()
	local bgSize = self.viewData.centerBg:getContentSize()
	local centerBgPos = cc.p(self.viewData.centerBg:getPositionX(), self.viewData.centerBg:getPositionY())
	...
	display.commonUIParams(teamFormationLabelBg, {po = cc.p(
		...
		centerBgPos.y + bgSize.height * 0.5 - 8)})
	self:addChild(teamFormationLabelBg, 10)

	local teamFormationLabel = display.newLabel(10, teamFormationLabelBg:getContentSize().height * 0.5,
		fontWithColor(3,{text = string.format(__('出战队伍%d'), 0), ap = display.LEFT_CENTER}))
	...
	-- 调整队伍
	local changeTeamFormationBtn = display.newButton(0, 0, {n = _res('ui/common/common_btn_orange.png'), scale9 = true, size = cc.size(142, 62)})
	...
	display.commonLabelParams(changeTeamFormationBtn, fontWithColor(14,{text = __('调整队伍')}))
	self:addChild(changeTeamFormationBtn, 10)
    local lsize = display.getLabelContentSize(changeTeamFormationBtn:getLabel())
    if lsize.width < 124 then lsize.width = 124 end
    changeTeamFormationBtn:setContentSize(cc.size(lsize.width + 8, 62))
...
--]]
function BattleReadyView:RefreshTeamFormation(data)
	------------ 处理编队数据 ------------
	local teamData = {}
...
刷新队伍周围信息
--]]
function BattleReadyView:RefreshTeamTabs()
...
刷新队伍选中状态
--]]
function BattleReadyView:RefreshTeamSelectedState(index)
	-- 刷新选中状态
...
--]]
function BattleReadyView:RefreshTeamInfo(teamData)
	-- 刷新头像
...
@params visible bool 是否显示
--]]
function BattleReadyView:ShowTeamFormationPanel(visible)
...
初始化堕神诱饵
--]]
function BattleReadyView:InitMagicFoodPanel()
	local bgSize = self.viewData.belongsBgFrame:getContentSize()
	local centerBgPos = cc.p(self.viewData.belongsBgFrame:getPositionX(), self.viewData.belongsBgFrame:getPositionY())
...
--]]
function BattleReadyView:RefreshMagicFoodState(magicFoodId)
...
@params visible bool 是否显示
--]]
function BattleReadyView:ShowMagicFoodPanel(visible)
	if visible then
		self:RefreshMagicFoodState(self.equipedMagicFoodId)
...
初始化关卡详情
--]]
function BattleReadyView:InitStageInfo()
	...
	local maxStarLabel = display.newLabel(
		...
		{text = __('三星条件'), fontSize = 26, color = '#bba496',reqW = 300 , ap = cc.p(0, 0.5)})
	...
	if QuestRechallenge.QR_CAN == checkint(stageConf.repeatChallenge) then
		...
		local sweepBtn = display.newButton(0, 0, {n = _res(btnPath), cb = handler(self, self.SweepBtnCallback)})
	...
	-- 关卡评论
 	local commentBtn = display.newButton(0, 0, {n = _res('ui/common/common_btn_orange.png'), cb = handler(self, self.commentBtnCallBack), scale9 = true, size = cc.size(124, 62)})
	...
	display.commonLabelParams(commentBtn, fontWithColor(14,{text = __('关卡评论')}))
	self:addChild(commentBtn, 111)
    local reviewWidth = display.getLabelContentSize(commentBtn:getLabel()).width
    if reviewWidth < 124 then reviewWidth = 124 end
    commentBtn:setContentSize(cc.size(reviewWidth + 8, 62))
	if stageConf then
		local questBattleType = CommonUtils.GetQuestBattleByQuestId(checkint(self.stageId))
		...
		if QuestRechallenge.QR_CAN == checkint(stageConf.repeatChallenge) and nil ~= stageConf.allClean then
			...
			for k,v in pairs(stageConf.allClean) do
				...
				local cleanLabel = display.newLabel(maxStarLabel:getPositionX(), splitLineStageInfo:getPositionY() - 30 - (itor - 1) * 35,
					{text = descr ,   fontSize = 22, color = cleanLabelColor, ap = cc.p(0, 0.5) ,reqW =550 })
				stageInfoBg:addChild(cleanLabel)
				if display.getLabelContentSize(cleanLabel).height /35 > 1 then
		...
		local iconScale = 0.2
		-- pass卡经验
		if app.passTicketMgr and app.passTicketMgr.GetTaskPointByModuleId then
			local point = app.passTicketMgr:GetTaskPointByQuestId(self.stageId)
			if point > 0 then
				local passTicketIcon = display.newNSprite(CommonUtils.GetGoodsIconPathById(PASS_TICKET_ID), 0, 0)
				passTicketIcon:setScale(iconScale)
				display.commonUIParams(passTicketIcon, {po = cc.p(splitLineStageReward:getPositionX() - 10, splitLineStageReward:getPositionY() + 20)})
				stageInfoBg:addChild(passTicketIcon)

				local passTicketLabel = display.newLabel(
					passTicketIcon:getPositionX() + passTicketIcon:getContentSize().width * 0.5 * iconScale + 5,
					passTicketIcon:getPositionY(),
					{text = tostring(point), fontSize = 22, color = '#ffffff', ap = cc.p(0, 0.5)})
				stageInfoBg:addChild(passTicketLabel)
			end
		end
		...
		if stageConf.firstPassMainExp and checkint(self.stageId) >= gameMgr:GetNewestQuestIdByDifficulty(checkint(stageConf.difficulty)) then
			...
			display.commonUIParams(firstLabel, {ap = cc.p(1, 0), po = cc.p(
				expIcon:getPositionX() + 60,
...
扫荡按钮回调
--]]
function BattleReadyView:SweepBtnCallback(sender)
	PlayAudioByClickNormal()
	AppFacade.GetInstance():DispatchObservers("SHOW_SWEEP_POPUP", {
		stageId = self.stageId
	})
end

function BattleReadyView:commentBtnCallBack(sender)
	PlayAudioByClickNormal()
...
上标签按钮点击回调
--]]
function BattleReadyView:ChangeCenterContentBtnCallback(sender)
...
@params index int 序号
--]]
function BattleReadyView:RefreshCenterContent(index)
	local curCenterTabBtn = self.viewData.centerContentTabBtns[index]
	if curCenterTabBtn then
		curCenterTabBtn:setChecked(true)
		local parentNode = curCenterTabBtn:getParent()
		display.commonLabelParams(parentNode:getChildByTag(3), {color = fontWithColor('13').color ,fontSize  =22 ,  reqW = 180 , w= 170 ,hAlign = display.TAC  })
	end
	...
	if preCenterTabBtn then
		...
		local parentNode = preCenterTabBtn:getParent()
		display.commonLabelParams(parentNode:getChildByTag(3), {color = fontWithColor('12').color,reqW = 170 ,fontSize  =22,  w = 170,hAlign = display.TAC })
	end
...
1002 后
--]]
function BattleReadyView:ChangeTeamFormationBtnCallback(sender)
	PlayAudioByClickNormal()
	local tag = sender:getTag()
	if 1001 == tag then
...
进入战斗
--]]
function BattleReadyView:EnterBattle(sender)
	------------ 本地逻辑判断 ------------
	-- 是否可以进入该关卡
	...
	if table.nums(selectedTeamData.members) == 0 then
		...
		elseif QuestBattleType.UNION_PARTY == self.questBattleType then
			...
			------------ 新手引导的逻辑 ------------
			if GUIDE_QUEST_SUCCESS_WORLD_MAP == self.stageId then
				-- 胜利后跳转至主界面
				if not app.gameMgr:JudgePassedStageByStageId(self.stageId) then
					fromToStruct.toMediatorName = 'HomeMediator'
				end
			end
			------------ 新手引导的逻辑 ------------
...
刷新剩余挑战次数
--]]
function BattleReadyView:RefreshChallengeTime()
...
--]]
function BattleReadyView:convertPlayerSkillData(allSkill)
	local skillId = 0
	local skillConf = nil
...
-- cocos2dx begin --
---------------------------------------------------
function BattleReadyView:onEnter()

end
function BattleReadyView:onExit()
...
-- debug raid begin --
---------------------------------------------------
function BattleReadyView:InitRaidDebug()
	self.debug = {}
	self.debugData = {
		players = {},
		cards = {},
	...
	questTeamIdInput:setPlaceHolder(__('房间id'))
	...
	questTeamIpInput:setPlaceHolder(__('房间ip'))
	...
	questTeamPortInput:setPlaceHolder(__('房间port'))
...
function BattleReadyView:DebugEnter(randomseed)
	local members = {}
	-- 获取有序位置
	...
	print('here check randomseed get from server>>>>>>>>>>>>', randomseed)
	dump(startRaidBattleData)
...
function BattleReadyView:AddTopCurrency( currency )
	local GoodPurchaseNode = require('common.GoodPurchaseNode')
	-- top icon
	local currencyBG = display.newImageView(_res('ui/home/nmain/main_bg_money'),0,0,{enable = false, scale9 = true, size = cc.size(680 + (display.width - display.SAFE_R),54)})
	...
	for i,v in ipairs(currency or {}) do
		local purchaseNode = GoodPurchaseNode.new({id = v, animate = false})
	...
	AppFacade.GetInstance():RegistObserver(SIGNALNAMES.CACHE_MONEY_UPDATE_UI, mvc.Observer.new(function (_, signal)
		...
		for id,v in pairs(moneyNodes) do
...
function BattleReadyView:onCleanup()
