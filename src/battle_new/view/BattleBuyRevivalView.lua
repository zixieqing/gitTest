...
@params table {
	...
	canBuyReviveFree bool 是否可以免费买活
}
--]]
...
function BattleBuyRevivalView:ctor( ... )
	...
	self.canBuyReviveFree = args.canBuyReviveFree
...
function BattleBuyRevivalView:GetCostConfig()
	...
	if true == self:CanBuyReviveFree() then
		-- 可以免费买活
		costGoodsAmount = 0
	end
...
是否可以免费买活
--]]
function BattleBuyRevivalView:CanBuyReviveFree()
	return self.canBuyReviveFree
end
---------------------------------------------------
-- get set end --
---------------------------------------------------
