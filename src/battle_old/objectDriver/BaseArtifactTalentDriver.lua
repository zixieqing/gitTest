...
function BaseArtifactTalentDriver:ctor( ... )
	...
	-- -- debug --
	-- if self.owner:getOCardId() == 200002 then
	-- 	self.talentData = {
	-- 		cardId = 200002,
	-- 		talentData = {
	-- 			['1'] = {
...
function BaseArtifactTalentDriver:InitTalentDataAdditions()
	...
	local cardId = self:GetOwner():getOCardId()
	...
	if nil ~= objTalentData then
		...
		for _, talentId_ in ipairs(sk) do
			...
			------------ 计算属性加成 天赋属性只有一种全部相加 ------------
			local talentptype, talentpvalue = artifactMgr.GetArtifactTalentInnateProperty(cardId, talentId, level, gemstoneId)
			...
			------------ 计算技能加成 ------------
			local skillId = artifactMgr.GetArtifactTalentInnateSkill(cardId, talentId, level, gemstoneId)
			if nil ~= skillId then
			...
			if nil ~= gemstoneId then
				local gemstonePAddition = artifactMgr.GetGemstonePropertyAddition(gemstoneId)
