...
function LineConnectGame:createATouchItem(drawName)
	...
	local dynamicCover = SpineCache(SpineCacheName.BATTLE):createWithName(sp.AniCacheName.CARD_CI_HEAD_ACTIVE)
	...
	if utils.isExistent(headPath) then
		...
		local headNode = display.newNSprite(headPath, 0, 0)
...
function LineConnectGame:initTouchItem()
	...
	for k,v in pairs(self.touchItems) do
		...
		-- 初始化方向
		v.towards = cc.p(
			math.random(G_BattleLogicMgr:GetBConf().cellSize.width * 5, G_BattleLogicMgr:GetBConf().cellSize.width * 10) * 0.01 * ((-1) ^ x[randomX]),
			math.random(G_BattleLogicMgr:GetBConf().cellSize.width * 5, G_BattleLogicMgr:GetBConf().cellSize.width * 10) * 0.01 * ((-1) ^ y[randomY]))
