...
local CapsuleNinePalaceView = class("CapsuleNinePalaceView", GameScene)
...
constructor
--]]
function CapsuleNinePalaceView:ctor(...)
	local args = unpack({...})
...
初始化ui
--]]
function CapsuleNinePalaceView:InitUI()
	...
	-- self:setBackgroundColor(cc.c4b(255, 128, 128, 255))
	...
	local function CreateView()
		...
		display.commonLabelParams(nextRoundButton, fontWithColor('4', {text = __('下一轮'), paddingW = 10}))
		...
		local drawButton = display.newButton(0, 0, {
			...
			scale9 = true , size = cc.size(280,80)
		})
...
初始化九宫格cell
--]]
function CapsuleNinePalaceView:InitNinePalaceCells()
...
获取一个普通的奖品九宫格预览
--]]
function CapsuleNinePalaceView:GetANormalNinePalaceCell()
		...
		RefreshGoodsIcon 		= function (self, goodsId, goodsAmount, viewComponent)
			...
			if nil == goodsIcon then
				...
				display.commonUIParams(goodsIcon, {ap = cc.p(0.5, 0.5), po = cc.p(
					...
					self.cellBgLayer:getContentSize().height * 0.5 + 20
				)})
				self.cellBgLayer:addChild(goodsIcon, 5)

				goodsNameLabel = display.newLabel(0, 0, fontWithColor('16', {text = goodsNameStr,w = 190 , hAlign= display.TAC ,  fontSize = 20}))
...
获取一个大奖的奖品九宫格预览
--]]
function CapsuleNinePalaceView:GetABigNinePalaceCell()
	...
	display.commonUIParams(bigLabel, {ap = cc.p(0.5, 0.5), po = cc.p(
		bigLabelBg:getContentSize().width * 0.5 - 4,
	...
	local cellData = {
		...
		RefreshGoodsIcon 		= function (self, goodsId, goodsAmount, viewComponent)
			...
			if nil == goodsIcon then
				...
				goodsNameLabel = display.newLabel(0, 0, fontWithColor('16', {text = goodsNameStr, fontSize = 20}))
				display.commonUIParams(goodsNameLabel, {ap = cc.p(0.5, 1), po = cc.p(
					goodsIcon:getPositionX(),
					goodsIcon:getPositionY() - goodsIcon:getContentSize().height * 0.5 - 5
				)})
...
刷一次中间的content位置
--]]
function CapsuleNinePalaceView:FixContentPosition()
...
--]]
function CapsuleNinePalaceView:RefreshUI(currentRound, totalRound, rewards, costGoodsId, costGoodsAmount)
	-- 刷新当前奖品数据
...
@params rewards list 奖品
--]]
function CapsuleNinePalaceView:RefreshRewardsData(rewards)
	-- 刷新缓存的奖品数据
...
--]]
function CapsuleNinePalaceView:RefreshCellSelectedMark(index)
...
--]]
function CapsuleNinePalaceView:RefreshRewardsPalaceCells(rewards)
...
--]]
function CapsuleNinePalaceView:RefreshDrawRoundLabel(currentRound)
...
--]]
function CapsuleNinePalaceView:RefreshCostInfo(goodsId, amount)
...
--]]
function CapsuleNinePalaceView:RefreshNextState(canEnterNextRound, currentRound, totalRound)
...
--]]
function CapsuleNinePalaceView:RefreshDrawState(hasRewards)
	if hasRewards then
...
--]]
function CapsuleNinePalaceView:DoDrawOneTime(squaredId)
...
--]]
function CapsuleNinePalaceView:DoDrawOneTimeByTargetCellIndex(targetCellIndex)
	-- 刷一遍格子 取出还未领取的格子序号
...
@params targetCellIndex int 目标格子序号
--]]
function CapsuleNinePalaceView:StartBanditAnimation(jumpList, enableCellIndex, targetCellIndex)
	-- 如果只剩一个格子了 不再做老虎机动画 直接结束
	if 0 >= #jumpList then
	...
	-- 去掉一个头
	table.remove(jumpList, #jumpList)

	local middleDelayTime = 0.05
	local startDelayTime = 0.25
	local startDeltaTime = (middleDelayTime - startDelayTime) / #enableCellIndex
	local delayTime = startDelayTime
	local itor = 0

	local function GotoNextCell(cellIndex)
		-- 刷新选中框状态
		self:RefreshCellSelectedMark(cellIndex)
		...
		------------ 处理变化的延迟时间 ------------
		if #enableCellIndex >= itor then
			delayTime = math.max(middleDelayTime, delayTime + startDeltaTime)
		elseif (BanditJumpRound - 1) * #enableCellIndex <= itor then
			delayTime = math.min(1.25, delayTime + math.random(400) / 10000 + 0.01)
		end
		------------ 处理变化的延迟时间 ------------

		-- print(cellIndex, itor, delayTime)

		if 0 < #jumpList then
			local nextCellIndex = jumpList[#jumpList]
			local actionSeq = cc.Sequence:create(
				cc.DelayTime:create(delayTime),
				cc.CallFunc:create(function ()
					GotoNextCell(nextCellIndex)
				end)
			)
	...
	local actionSeq = cc.Sequence:create(
		cc.DelayTime:create(delayTime),
		cc.CallFunc:create(function ()
			GotoNextCell(jumpList[#jumpList])
		end)
	)
	self:runAction(actionSeq)
end
...
@params targetCellIndex int 目标格子序号
--]]
function CapsuleNinePalaceView:OverBanditAnimation(targetCellIndex)
...
开始刷新下一波
--]]
function CapsuleNinePalaceView:DoEnterNextRound()
	-- 清除获奖spine
	self:ClearRewardSpineNode()
	-- 清除选中框状态
	self:SetCurrentSelectedIndex(nil)
...
转结束 开始散开
--]]
function CapsuleNinePalaceView:OverEnterNextRound()
	local centerPos = cc.p(
		self.viewData.centerDrawBg:getPositionX(),
		self.viewData.centerDrawBg:getPositionY()
	)
...
function CapsuleNinePalaceView:GetFixedBezierPos(p1, p2, len)
	-- p1起点 p2终点
	local p = cc.p(0, 0)
...
抽奖按钮回调
--]]
function CapsuleNinePalaceView:DrawOneClickHandler(sender)
...
下一波按钮回调
--]]
function CapsuleNinePalaceView:NextRoundClickHandler(sender)
	PlayAudioByClickNormal()
...
@return _ cellData
--]]
function CapsuleNinePalaceView:GetCellDataByCellIndex(cellIndex)
...
@return _ int index
--]]
function CapsuleNinePalaceView:GetIndexByCellIndex(cellIndex)
	return self.rewardsFixedIndex[cellIndex]
end
--[[
是否还有能抽的奖品
--]]
function CapsuleNinePalaceView:HasRewards()
	return self.hasRewards
end
function CapsuleNinePalaceView:SetHasRewards(has)
	self.hasRewards = has
end
--[[
根据序号获取reward信息
...
@return _ map
--]]
function CapsuleNinePalaceView:GetRewardDataByIndex(index)
...
@return _ map
--]]
function CapsuleNinePalaceView:GetRewardDataByCellIndex(cellIndex)
...
--]]
function CapsuleNinePalaceView:GetCellIndexBySquaredId(squaredId)
	local rewardData = nil
	for cellIndex, index in ipairs(self.rewardsFixedIndex) do
...
获取当前选中框停留的index
--]]
function CapsuleNinePalaceView:GetCurrentSelectedIndex()
	return self.selectedCellIndex
end
function CapsuleNinePalaceView:SetCurrentSelectedIndex(index)
	self.selectedCellIndex = index
end
--[[
获取是否抽到过大奖
--]]
function CapsuleNinePalaceView:HasDrawnBigReward()
	return self.hasDrawnBigReward
end
function CapsuleNinePalaceView:SetHasDrawnBigReward(has)
	self.hasDrawnBigReward = has
end
--[[
获取当前逻辑上是否满足能进入下一轮
--]]
function CapsuleNinePalaceView:CanEnterNextRound()
...
获取得奖的spine
--]]
function CapsuleNinePalaceView:GetRewardSpineNode()
	return self.rewardSpineAni
end
function CapsuleNinePalaceView:SetRewardSpineNode(spineNode)
	self.rewardSpineAni = spineNode
end
function CapsuleNinePalaceView:ClearRewardSpineNode()
...
@return _ cc.p坐标
--]]
function CapsuleNinePalaceView:GetCellPositionByCellIndex(cellIndex)
	local centerPos =  cc.p(
		self.viewData.centerDrawBg:getPositionX(),
		self.viewData.centerDrawBg:getPositionY()
	)
...
--]]
function CapsuleNinePalaceView:GetJumpListByEnableCellIndex(enableCellIndex, startCellIndex, overCellIndex)
	local jumpList = {}
...
获取当前活动id
--]]
function CapsuleNinePalaceView:GetActivityId()
	return self.activityId
end
function CapsuleNinePalaceView:SetActivityId(activityId)
...
--]]
function CapsuleNinePalaceView:GetSignalParams(params)
	if nil == params then
		params = {}
	end
...
-- cocos2dx --
---------------------------------------------------
function CapsuleNinePalaceView:onEnter()
