...
local PointSettleView = class('PointSettleView', BattleSuccessView)
...
初始化ui
--]]
function PointSettleView:InitUI()
	...
	-- 初始化点数层
	local pointLayer = self:InitPointLayer()
	table.insert(self.layers, pointLayer)
...
初始化公共层
--]]
function PointSettleView:InitCommonLayer()
...
初始化结算结果层
--]]
function PointSettleView:InitBattleResultLayer()
	local layerSize = cc.size(display.SAFE_RECT.width, display.SAFE_RECT.height)
		...
		local mainSpineNodeActionSeq = cc.Sequence:create(
			...
			cc.CallFunc:create(function ()
				...
				if BattleResult.BR_SUCCESS == self.battleResult then
					-- 播放胜利音效
					PlayUIEffects(AUDIOS.UI.ui_war_win.id)
				else
					-- 播放失败音效
					PlayUIEffects(AUDIOS.UI.ui_war_lose.id)
...
添加结果层的底部按钮
--]]
function PointSettleView:AddBattleResultBottomLayer(parentNode)
	-- 存在下一层
...
初始化经验层
--]]
function PointSettleView:InitExpLayer()
	local layerSize = cc.size(display.SAFE_RECT.width, display.SAFE_RECT.height)
...
初始化道具掉落层
--]]
function PointSettleView:InitRewardsLayer()
...
初始化点数层
--]]
function PointSettleView:InitPointLayer()
	if nil == self.rewardsBg then return {} end

	local statisticsInfo = {}

	if true then
		local currentSummerPoint = 0
		local deltaSummerPoint = 0
		local hasSummerPoint = 0

		if nil ~= self.trophyData.summerPoint then

			-- 夏活的点数
			currentSummerPoint = checkint(self.trophyData.summerPoint)
			deltaSummerPoint = checkint(self.trophyData.deltaSummerPoint)
			hasSummerPoint = currentSummerPoint - deltaSummerPoint

		-- TODO --
		elseif QuestBattleType.PT_DUNGEON == CommonUtils.GetQuestBattleByQuestId(checkint(self.trophyData.requestData.questId)) then

			currentSummerPoint = checkint(self.trophyData.point)
			hasSummerPoint = app.ptDungeonMgr:GetPoint()
			deltaSummerPoint = currentSummerPoint - hasSummerPoint
		-- TODO --

		elseif nil ~= self.trophyData.point then

			-- 通用的点数
			currentSummerPoint = checkint(self.trophyData.point)
			deltaSummerPoint = checkint(self.trophyData.deltaPoint)
			hasSummerPoint = currentSummerPoint - deltaSummerPoint

		end

		statisticsInfo = {
			{title = __('新增点数'), value = deltaSummerPoint, str = tostring(deltaSummerPoint)},
			{title = __('已有点数'), value = hasSummerPoint, str = tostring(hasSummerPoint)},
			{title = __('共计点数'), value = currentSummerPoint, str = tostring(currentSummerPoint)}
		}
	end
	...
	local layer = {ShowSelf = ShowSelf, HideSelf = HideSelf, layerName = 'InitPointLayer'}
	return layer
end
---------------------------------------------------
-- init end --
...
更新本地数据
--]]
function PointSettleView:UpdateLocalData()
...
---------------------------------------------------
function PointSettleView:onEnter()
	self:ShowNextLayer()
	self:ShowNextLayer()
end
---------------------------------------------------
