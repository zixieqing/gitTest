...
local NAME = 'drawCards.CapsuleMallMediaor'
local CapsuleMallMediaor = class(NAME, mvc.Mediator)
...
function CapsuleMallMediaor:ctor(params, viewComponent)
...
function CapsuleMallMediaor:Initial(key)
    ...
    -- self:initData(self.ctorArgs_)

    self:initView()
    
end

function CapsuleMallMediaor:cleanupView()
...
function CapsuleMallMediaor:OnRegist()
    -- app:DispatchObservers(HomeScene_ChangeCenterContainer, 'rightHide')
    regPost(POST.GAMBLING_SKIN_MALL)
    regPost(POST.GAMBLING_SKIN_MALL_BUY)
    self:EnterLayer()
end


function CapsuleMallMediaor:OnUnRegist()
    -- app:DispatchObservers(HomeScene_ChangeCenterContainer, 'hide')
    unregPost(POST.GAMBLING_SKIN_MALL)
    unregPost(POST.GAMBLING_SKIN_MALL_BUY)
    self:cleanupView()
end


function CapsuleMallMediaor:InterestSignals()
    local signalList = {
        POST.GAMBLING_SKIN_MALL.sglName,
...
function CapsuleMallMediaor:ProcessSignal(signal)
    local name = signal:GetName()
    local data = signal:GetBody() or {}
    if name == POST.GAMBLING_SKIN_MALL.sglName then

        self:initData(data)
        self:initExpandableListView()
        self:refreshUI()

    elseif name == POST.GAMBLING_SKIN_MALL_BUY.sglName then
        local requsetData = data.requestData or {}
        if requsetData.activityId ~= self.activityId then return end
    ...
    elseif name == 'CAPSULE_MALL_GOOD_BUY' then
        self.buyData = data
        local params = {activityId = self.activityId, productId = data.productId, productNum = data.productNum, goodsId = data.goodsId}
        -- app:DispatchObservers(POST.GAMBLING_SKIN_MALL_BUY.sglName, {requestData = params, rewards = {{goodsId = data.goodsId, num = data.productNum}}})
        self:SendSignal(POST.GAMBLING_SKIN_MALL_BUY.cmdName, params)
    end
...
-------------------------------------------------
-- init
function CapsuleMallMediaor:initValue()
...
function CapsuleMallMediaor:initData(datas)
    local mallDatas = {}
    local tabList = {}
...
function CapsuleMallMediaor:initExpandableListView()
    ...
    for _, tabData in ipairs(self.tabList) do
        ...
        if btn then
            commonLabelParams(btn, {text = tostring(mallTypeConf.name) , w = 130 , hAlign = display.TAC , reqH= 65 })
...
function CapsuleMallMediaor:getViewData()
    return self.viewData_
end

function CapsuleMallMediaor:getOwnerScene()
    return self.ownerScene_
end

function CapsuleMallMediaor:getMallTypeByGoodsId(goodTypeConf, goodsId)
    return goodTypeConf[CommonUtils.GetGoodTypeById(goodsId)]
end

function CapsuleMallMediaor:getMdtConfByMallType(mallType)
    return MALL_TYPE_DEFINE[mallType]
end

function CapsuleMallMediaor:getMdtDataByMallType(mallType)
...
function CapsuleMallMediaor:EnterLayer()
    self:SendSignal(POST.GAMBLING_SKIN_MALL.cmdName, {activityId = self.activityId})
end

function CapsuleMallMediaor:updateMallDataByMallType(mallType, params)
...
function CapsuleMallMediaor:refreshUI()
    self:refreshExpandableListView()
    self:refreshChildMediaor()
end

function CapsuleMallMediaor:refreshExpandableListView()
    local viewData = self:getViewData()
    local expandableListView = viewData.expandableListView
    local viewComponent = self:GetViewComponent()
...
function CapsuleMallMediaor:refreshChildMediaor()
...
-------------------------------------------------
-- handler
function CapsuleMallMediaor:onClickCellAction(sender)
...
function CapsuleMallMediaor:onClickItemAction(sender)
    PlayAudioByClickNormal()
    local mallType = sender:getUserTag()
    if self.selectedMallType == mallType then return end
...
function CapsuleMallMediaor:onClickBackBtnAction(sender)
    app:UnRegsitMediator(self.contentMdtName)
    app:UnRegsitMediator(NAME)
end

return CapsuleMallMediaor
